...
use yunpat_execpolicy::{AskForApproval, ExecPolicyContext, ExecPolicyEngine};
use yunpat_mcp::{McpServerDefinition, run_stdio_server};
use yunpat_secrets::Secrets;
use yunpat_state::{StateStore, ThreadListFilters};
...
fn provider_env_set(provider: ProviderKind) -> bool {
    yunpat_secrets::env_for(provider_slot(provider)).is_some()
}
...
mod tests {
    ...
    #[test]
    fn yunpat_login_writes_shared_config_and_preserves_tui_defaults() {
    ...
    fn auth_set_writes_to_shared_config_file() {
        use std::sync::Arc;
        use yunpat_secrets::{InMemoryKeyringStore, KeyringStore};
    ...
    fn auth_status_and_list_only_probe_active_provider_keyring() {
        ...
        use yunpat_secrets::{KeyringStore, SecretsError};
