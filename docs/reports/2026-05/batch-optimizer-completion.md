...
interface ITokenCounter {
  ...
  truncateToTokenLimit(text: string, model: string, maxTokens: number, safetyMargin?: number): string;
}
```
...
class BatchProcessorOptimizer {
  ...
  partitionIntoBatches(texts: string[], model: string, maxBatchSize?: number): BatchOptimizationResult;
