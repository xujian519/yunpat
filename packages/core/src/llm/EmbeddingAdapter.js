...
export class EmbeddingAdapter extends BaseEmbeddingProvider {
    ...
    async embed(params) {
        ...
        try {
            ...
            throw new EmbeddingError(`嵌入失败: ${error instanceof Error ? error.message : String(error)}`, EmbeddingErrorCode.API_ERROR, this.config.model);
        }
    }
    /**
     * 嵌入单个文本
    ...
    async embedBatch(texts) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new EmbeddingError(`API 请求失败: ${response.status} ${errorText}`, EmbeddingErrorCode.API_ERROR, this.config.model);
            }
            const data = (await response.json());
            // 验证响应格式
            if (!data.data || !Array.isArray(data.data)) {
                throw new EmbeddingError('API 返回格式异常: 缺少 data 字段', EmbeddingErrorCode.API_ERROR, this.config.model);
            }
            // 按 index 排序确保顺序正确
            ...
            // 网络错误或其他异常
            throw new EmbeddingError(`网络请求失败: ${error instanceof Error ? error.message : String(error)}`, EmbeddingErrorCode.API_ERROR, this.config.model);
        }
    }
    /**
     * 更新能力配置
