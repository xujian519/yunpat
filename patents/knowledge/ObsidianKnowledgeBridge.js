...
export class ObsidianKnowledgeBridge {
    ...
    async getWikiPage(pagePath) {
        ...
        // 提取所有wikilink
        const links = (content.match(/\[\[([^\]]+)\]\]/g) || []).map((link) => link.replace(/[\[\]]/g, ''));
        const page = {
            path: pagePath,
            title,
            content,
    ...
    parseCard(content) {
        ...
        const cardContent = lines
            ...
            .trim();
        // 提取相关页面（[[wikilink]]）
        const relatedPages = (content.match(/\[\[([^\]]+)\]\]/g) || []).map((link) => link.replace(/[\[\]]/g, ''));
        return {
            question,
            quality,
            content: cardContent,
     ...
     * @returns 提取的问题
     */
    extractQuestionFromFilename(filename) {
     ...
     * @returns 相关页面的路径数组
     */
    parseConceptIndex(indexContent, concept) {
