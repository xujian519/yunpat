...
impl App {
    ...
    pub fn new(options: TuiOptions, config: &Config) -> Self {
        ...
        let (ui_theme, theme_mode) = {
            let prefs = crate::settings::TuiPrefs::load().unwrap_or_default();
            let mode = match prefs.theme.as_str() {
                "light" => palette::PaletteMode::Light,
                "dark" => palette::PaletteMode::Dark,
                _ => palette::PaletteMode::detect(),
            };
            (palette::UiTheme::for_mode(mode), mode)
        };
        ...
        Self {
            ...
            router: yunpat_router::Router::new(),
        }
    }
...
pub enum AppAction {
    ...
    /// Launch a patent workflow agent.
    PatentWorkflow {
        agent_id: String,
        topic: Option<String>,
