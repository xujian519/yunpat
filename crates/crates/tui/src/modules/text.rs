...
pub async fn run_yunpat_chat(client: &dyn LlmClient, options: TextChatOptions) -> Result<()> {
    ...
    if let Some(prompt) = options.prompt.as_deref() {
        process_yunpat_turn(client, &options, &mut messages, prompt, &mut stats).await?;
    } else {
...
pub async fn run_official_chat(client: &dyn LlmClient, options: TextChatOptions) -> Result<()> {
...
async fn handle_line_deepseek(
    ...
    if let Err(error) = process_yunpat_turn(client, options, messages, input, stats).await {
