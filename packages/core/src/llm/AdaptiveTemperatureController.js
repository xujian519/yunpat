...
export class AdaptiveTemperatureController {
     ...
     */
    getThermalParams(task, customBaseTemp) {
        const complexity = this.evaluateComplexity(task);
        let temperature = customBaseTemp ?? this.baseTemperatures.get(complexity) ?? this.config.mediumTemperature;
        // 应用历史反馈的累积调整
        temperature = this.applyHistoryAdjustment(temperature, complexity);
        // 确保 temperature 在范围内
        temperature = Math.max(this.config.minTemperature, Math.min(this.config.maxTemperature, temperature));
     ...
     * @returns 调整后的温度
     */
    adjustTemperature(currentTemp, feedback, complexity) {
     ...
     * 应用历史反馈的累积调整
     */
    applyHistoryAdjustment(baseTemp, complexity) {
        // 获取最近的相同复杂度的历史记录
     ...
     * 生成原因说明
     */
    getReasonString(complexity, temperature) {
     ...
     * 记录温度历史
     */
    recordHistory(complexity, temperature, feedback) {
        this.history.push({
            timestamp: new Date(),
            complexity,
            temperature,
 ...
 * @returns 控制器实例
 */
export function createAdaptiveTemperatureController(config) {
