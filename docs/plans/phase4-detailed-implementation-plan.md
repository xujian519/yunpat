...
> {
  ...
  private async call5_ExceptionHandling(error: Error, context: ExecutionContext): Promise<RecoveryResult>
}
```
    ...
    for (const layer of dag.layers) {
      if (layer.length === 1) {
        ...
        const layerResults = await Promise.all(
          layer.map(step => this.executeStep(step, context))
        )
...
class EventBus {
  ...
  async emitAll(events: { type: string; payload: any }[]): Promise<void> {
    await Promise.all(
      events.map(e => this.emit(e.type, e.payload))
    )
  }
...
class AgentRegistry {
  ...
  async dispatchParallel(requests: Array<{
    agentId: string
    input: any
    context: ExecutionContext
  }>): Promise<AgentResult[]> {
    return Promise.all(
      requests.map(req => this.dispatch(req.agentId, req.input, req.context))
    )
  }
}
```
...
describe('Agent Integration Tests', () => {
  it('should dispatch task to PatentWriterAgent', async () => {
    const result = await registry.dispatch('patent-writer', {
      task: 'draft-claims',
      input: { /* ... */ }
    }, context)
  ...
  it('should dispatch parallel tasks', async () => {
    const results = await registry.dispatchParallel([
      { agentId: 'search-agent', input: { /* ... */ }, context },
      { agentId: 'knowledge-agent', input: { /* ... */ }, context }
    ])
