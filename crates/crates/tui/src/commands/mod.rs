...
pub const COMMANDS: &[CommandInfo] = &[
    ...
    CommandInfo {
        ...
        aliases: &[],
        usage: "/draft [topic]",
        description_id: MessageId::CmdDraftDescription,
    },
    CommandInfo {
        name: "oa",
        aliases: &[],
    ...
    CommandInfo {
        name: "trademark",
        aliases: &["tm"],
        usage: "/trademark [topic]",
        description_id: MessageId::CmdTrademarkDescription,
    },
    CommandInfo {
        name: "creativity",
        aliases: &["inv", "inventive"],
        usage: "/creativity [topic]",
        description_id: MessageId::CmdCreativityDescription,
    },
    CommandInfo {
        name: "case",
        aliases: &["cases"],
        usage: "/case <new|list|show|doc|archive> [args]",
        description_id: MessageId::CmdCaseDescription,
    },
    CommandInfo {
        name: "kb",
        aliases: &[],
        usage: "/kb <add|search|list|remove> [args]",
        description_id: MessageId::CmdKbDescription,
    },
    CommandInfo {
        name: "patent-search",
        aliases: &["ps"],
        usage: "/patent-search <query>",
        description_id: MessageId::CmdPatentSearchDescription,
    },
    CommandInfo {
        name: "paper-search",
        aliases: &["ss"],
        usage: "/paper-search <query>",
        description_id: MessageId::CmdPaperSearchDescription,
    },
    CommandInfo {
        name: "patent-db",
        aliases: &["pdb"],
        usage: "/patent-db <query> [--ipc <code>] [--applicant <name>]",
        description_id: MessageId::CmdPatentDbDescription,
    },
    CommandInfo {
        name: "ocr",
        aliases: &[],
        usage: "/ocr <image_path> [language]",
        description_id: MessageId::CmdOcrDescription,
    },
    CommandInfo {
        name: "docx",
        aliases: &[],
        usage: "/docx <markdown_path> [output_path]",
        description_id: MessageId::CmdDocxDescription,
    },
];
...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    match command {
        ...
        "links" | "dashboard" | "api" => core::yunpat_links(app),
        ...
        // Patent workflow commands
        "research" => patent::research(app, arg),
        "draft" => patent::draft(app, arg),
        "oa" => patent::oa(app, arg),
        "reexam" => patent::reexam(app, arg),
        "invalid" => patent::invalid(app, arg),
        "trademark" | "tm" => patent::trademark(app, arg),
        "creativity" | "inv" | "inventive" => patent::creativity(app, arg),
        "case" | "cases" => case::case(app, arg),
        "kb" => kb::kb(app, arg),
        "patent-search" | "ps" => patent::patent_search(app, arg),
        "paper-search" | "ss" => patent::paper_search(app, arg),
        "patent-db" | "pdb" => patent::patent_db(app, arg),
        "ocr" => patent::ocr_command(app, arg),
        "docx" => patent::docx_command(app, arg),
...
mod tests {
    ...
    fn removed_set_and_yunpat_commands_show_migration_hints() {
        ...
        assert!(yunpat_msg.contains("/api"));
        assert!(yunpat_result.action.is_none());
    }
    ...
    #[test]
    fn e2e_patent_search_command_parses_and_dispatches() {
        let mut app = create_test_app();
        let result = execute("/patent-search artificial intelligence", &mut app);

        assert!(!result.is_error, "命令应该成功执行");
        assert!(result.message.is_some(), "应该返回消息");
        let msg = result.message.unwrap();
        assert!(
            msg.contains("Searching patents"),
            "消息应包含搜索提示: {}",
            msg
        );
        ...
        match result.action {
            ...
            other => panic!("期望 PatentWorkflow action，得到 {:?}", other),
        }
    }

    #[test]
    fn e2e_patent_search_with_alias_ps() {
        let mut app = create_test_app();
        let result = execute("/ps machine learning", &mut app);

        assert!(!result.is_error);
        match result.action {
            Some(AppAction::PatentWorkflow { agent_id, .. }) => {
                assert_eq!(agent_id, "patent_search", "别名 ps 应解析为 patent-search");
            }
            _ => panic!("别名 ps 应解析为 patent-search"),
        }
    }

    #[test]
    fn e2e_paper_search_command_parses_and_dispatches() {
        let mut app = create_test_app();
        let result = execute("/paper-search transformer", &mut app);
        ...
        match result.action {
            ...
            _ => panic!("期望 PaperSearch Workflow"),
        }
    }

    #[test]
    fn e2e_paper_search_with_alias_ss() {
        let mut app = create_test_app();
        let result = execute("/ss deep learning", &mut app);

        assert!(!result.is_error);
        match result.action {
            Some(AppAction::PatentWorkflow { agent_id, .. }) => {
                assert_eq!(agent_id, "paper_search", "别名 ss 应解析为 paper-search");
            }
            _ => panic!("别名 ss 应解析为 paper-search"),
        }
    }

    #[test]
    fn e2e_patent_db_command_with_keyword() {
        let mut app = create_test_app();
        let result = execute("/patent-db 人工智能", &mut app);
        ...
        match result.action {
            ...
            _ => panic!("期望 PatentDb Workflow"),
        }
    }

    #[test]
    fn e2e_patent_db_command_with_ipc_filter() {
        let mut app = create_test_app();
        let result = execute("/patent-db --ipc G06N", &mut app);
    ...
    #[test]
    fn e2e_patent_db_command_with_applicant_filter() {
        let mut app = create_test_app();
        let result = execute("/patent-db --applicant 华为", &mut app);
    ...
    #[test]
    fn e2e_patent_db_command_without_query_shows_error() {
        let mut app = create_test_app();
        let result = execute("/patent-db", &mut app);

        assert!(result.is_error, "无参数时应返回错误");
        assert!(
            result.message.as_ref().unwrap().contains("Usage"),
            "错误消息应包含用法提示"
        );
    }

    #[test]
    fn e2e_creativity_command_parses_and_dispatches() {
        let mut app = create_test_app();
        let result = execute("/creativity 创造性判断分析", &mut app);
        ...
        match result.action {
            ...
            _ => panic!("期望 Creativity Workflow"),
        }
    }

    #[test]
    fn e2e_creativity_command_with_alias_inv() {
        let mut app = create_test_app();
        let result = execute("/inv 新颖性分析", &mut app);

        assert!(!result.is_error);
        match result.action {
            Some(AppAction::PatentWorkflow { agent_id, .. }) => {
                assert_eq!(agent_id, "creativity", "别名 inv 应解析为 creativity");
            }
            _ => panic!("别名 inv 应解析为 creativity"),
        }
    }

    #[test]
    fn e2e_research_command_parses_and_dispatches() {
        let mut app = create_test_app();
        let result = execute("/research 专利创造性判定规则", &mut app);
        ...
        match result.action {
            Some(AppAction::PatentWorkflow {
                ...
                assert_eq!(
                    topic,
                    Some("专利创造性判定规则".to_string()),
                    "主题应正确传递"
                );
            }
            _ => panic!("期望 Research Workflow"),
        }
    }

    #[test]
    fn e2e_all_patent_commands_have_localized_descriptions() {
        use crate::localization::{Locale, MessageId, tr};
        ...
        let message_ids = [
            ...
            MessageId::CmdTrademarkDescription,
            MessageId::CmdCreativityDescription,
            MessageId::CmdPatentSearchDescription,
            MessageId::CmdPaperSearchDescription,
