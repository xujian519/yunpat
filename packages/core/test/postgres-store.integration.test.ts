...
import {
  PostgresVectorStore,
  type SearchFilter,
} from '../src/memory/long-term/PostgresVectorStore.js'
...
describe.skipIf(!process.env.TEST_DATABASE_URL && !process.env.CI)(
  ...
  () => {
    ...
    beforeAll(async () => {
      // 从环境变量读取数据库 URL
      const databaseUrl =
        process.env.TEST_DATABASE_URL ?? 'postgres://yunpat:yunpat123@localhost:5432/yunpat_test'
    ...
    describe('边界情况和错误处理', () => {
      it('应该拒绝维度不匹配的向量', async () => {
        await expect(vectorStore.search(Array(512).fill(0.1), 5)).rejects.toThrow('向量维度不匹配')
      })
