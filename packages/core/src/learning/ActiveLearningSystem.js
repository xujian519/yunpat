...
export class ActiveLearningSystem {
     ...
     * @returns 不确定性分数
     */
    estimateUncertainty(result, method = 'entropy') {
        // 预测不确定性 - 基于模型输出的置信度
     ...
     * 估计预测不确定性
     */
    estimatePredictionUncertainty(result, method) {
        // 从结果中提取置信度
     ...
     * 基于输入的模糊、歧义程度
     */
    estimateSemanticUncertainty(result) {
        let uncertainty = 0;
     ...
     * 基于多次预测的一致性
     */
    estimateDistributionUncertainty(predictions) {
     ...
     * 比较两个预测的差异
     */
    comparePredictions(pred1, pred2) {
        // 简化实现：基于字符串相似度
     ...
     * 计算总体不确定性
     */
    calculateTotalUncertainty(prediction, semantic, distribution) {
        // 加权平均
     ...
     * @returns 选中的样本列表
     */
    selectSamplesForAnnotation(budget) {
     ...
     * 选择预计对模型影响最大的样本
     */
    expectedModelChangeSampling(budget) {
        // 简化实现：结合不确定性和样本新颖性
        const samples = Array.from(this.samplePool.values()).map((s) => {
            // 新颖性：与已标注样本的平均相似度
            let novelty = 1;
            if (this.annotatedSamples.size > 0) {
                const similarities = Array.from(this.annotatedSamples.values()).map((a) => this.calculateSimilarity(s, a));
     ...
     * @returns 标注结果
     */
    async annotateWithHuman(sample, annotator) {
     ...
     */
    addAnnotation(sampleId, annotation) {
     ...
     * 模拟模型更新
     */
    async simulateModelUpdate(trainingData) {
        // 实际应用中，这里会调用：
        // - 模型微调 API
        // - 知识库更新
        // - 提示词优化
    ...
    calculateLearningEfficiency() {
        ...
        const efficiency = (estimatedPassiveSamples - this.metrics.totalAnnotated) / estimatedPassiveSamples;
     ...
     * @returns A/B 测试结果
     */
    async runABTest(controlData, treatmentData) {
        // 计算对照组准确率
     ...
     * 评估最近一次更新的效果
     */
    async incrementalEvaluate() {
        const accuracy = this.metrics.currentAccuracy;
        const gain = this.metrics.accuracyTrend.length >= 2
     ...
     * 添加样本到池
     */
    addSample(result, source = 'prediction') {
     ...
     * 批量添加样本
     */
    addSamples(results, source = 'prediction') {
        return results.map((r) => this.addSample(r, source));
    }
    /**
     * 触发主动学习
     */
    triggerActiveLearning(sample) {
        // 发布事件，请求外部系统处理
        // 实际应用中，这里会调用 EventBus 发布事件
        console.log(`[ActiveLearning] High uncertainty detected: ${sample.id} (${sample.uncertaintyScore})`);
    }
    // ==================== 学习循环 ====================
    /**
     * 运行一轮主动学习
 ...
 * 创建默认配置
 */
export function createDefaultActiveLearningConfig() {
    return {
 ...
 * 创建主动学习系统
 */
export function createActiveLearningSystem(llm, config) {
    const fullConfig = {
