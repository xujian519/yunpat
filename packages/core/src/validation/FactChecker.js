 ...
 */
import { ClaimCategory, SourceType, } from './hallucination-types.js';
import { ExternalFactChecker, aggregateResults, FactCheckError, } from './ExternalFactChecker.js';
/**
 * 事实验证器
 */
export class FactChecker {
    ...
    constructor(llm, knowledgeBase, config) {
        ...
        if (this.config.verificationMethods.includes('external_api')) {
            this.externalChecker = new ExternalFactChecker(this.config.externalAPIConfig);
        }
    }
    /**
     * 验证内容中的所有声明
    ...
    async extractClaims(content) {
        ...
        if (this.config.extractionMethod === 'regex') {
            ...
            for (const llmClaim of llmClaims) {
                const isDuplicate = regexClaims.some((rc) => rc.content.toLowerCase() === llmClaim.content.toLowerCase());
                if (!isDuplicate) {
     ...
     * @returns 提取的声明列表
     */
    async extractClaimsByLLM(content) {
...
只返回JSON，不要有其他内容。`;
     ...
     * @returns 事实验证结果
     */
    async verifyWithKnowledgeBase(claim) {
        try {
            // 在知识库中搜索相关内容
     ...
     * @returns 事实验证结果
     */
    async verifyWithExternalAPI(claim, options) {
     ...
     * @returns 聚合验证结果
     */
    async verifyCrossSources(claim, options) {
        const results = [];
        // 1. 知识库验证
        try {
     ...
     */
    async getKnowledgeBaseResult(claim) {
     ...
     * @returns 验证结果列表
     */
    async verifyClaimsWithExternalAPI(claims, options) {
        if (!this.externalChecker) {
            throw new FactCheckError('外部 API 未启用', undefined, 'FactChecker');
        }
        const externalResults = await this.externalChecker.verifyClaims(claims.map((c) => c.content), options);
