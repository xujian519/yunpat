...
impl Default for CapacityControllerConfig {
    fn default() -> Self {
        ...
        model_priors.insert("yunpat_v3_2_chat".to_string(), 3.9);
        model_priors.insert("yunpat_v3_2_reasoner".to_string(), 4.1);
        model_priors.insert("yunpat_v4_pro".to_string(), 3.5);
        model_priors.insert("yunpat_v4_flash".to_string(), 4.2);
...
impl CapacityControllerConfig {
    ...
    pub fn from_app_config(config: &crate::config::Config) -> Self {
        ...
        if let Some(v) = capacity.yunpat_v3_2_chat_prior {
        ...
        if let Some(v) = capacity.yunpat_v4_flash_prior {
            out.model_priors.insert("yunpat_v4_flash".to_string(), v);
        }
...
mod tests {
    ...
    fn app_config_without_capacity_uses_default_disabled() {
        ...
        assert_eq!(cfg.model_priors.get("yunpat_v4_pro"), Some(&3.5));
        assert_eq!(cfg.model_priors.get("yunpat_v4_flash"), Some(&4.2));
    }
    ...
    fn v4_priors_loaded_into_default_config() {
        let cfg = CapacityControllerConfig::default();
        assert_eq!(cfg.model_priors.get("yunpat_v4_pro").copied(), Some(3.5));
        assert_eq!(cfg.model_priors.get("yunpat_v4_flash").copied(), Some(4.2));
    }
