...
//!
//! As of v0.8.x the engine has been refactored (#402) — it now holds
//! `Option<Arc<dyn LlmClient>>` and the `LlmClient` trait is object-safe.
//! The 4 engine-level tests below are kept `#[ignore]` because they still
//! need actual test logic (currently stubs). Once implemented the mock
//! injects directly into the engine constructor.
...
// === 9. Engine integration (pending implementation) =========================
//
// #402 is resolved — the engine now uses `Arc<dyn LlmClient>` and a mock
// can be injected. These tests remain `#[ignore]` because the test bodies
// are stubs; each needs actual engine construction + turn-driving logic.

#[tokio::test]
#[ignore = "#402 resolved: needs test implementation"]
