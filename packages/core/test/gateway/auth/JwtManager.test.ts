...
describe('JwtManager', () => {
  ...
  describe('verifyAccessToken', () => {
    it('应该验证有效的 Token', async () => {
      const tokenPair = await jwtManager.generateTokenPair('user-123', ['user'], ['read'])
  ...
  describe('revokeToken', () => {
    ...
    it('撤销不存在的 Token 不应该抛出错误', async () => {
      await expect(jwtManager.revokeToken('non-existent-token')).resolves.toBeUndefined()
    })
  })
  ...
  describe('refreshTokens', () => {
    it('应该使用 UserDataProvider 刷新 Token', async () => {
      ...
      // 刷新 Token
      const refreshedTokens = await managerWithProvider.refreshTokens(initialTokens.refreshToken)
      ...
      const result = await managerWithProvider.verifyAccessToken(refreshedTokens!.accessToken)
    ...
    it('如果没有 UserDataProvider 应该使用默认值', async () => {
      ...
      // 刷新 Token（应该使用默认值）
      const refreshedTokens = await managerWithoutProvider.refreshTokens(initialTokens.refreshToken)

      expect(refreshedTokens).not.toBeNull()

      // 验证新 Token 包含默认值
      const result = await managerWithoutProvider.verifyAccessToken(refreshedTokens!.accessToken)
    ...
    it('如果刷新 Token 无效应该返回 null', async () => {
      ...
      // 使用无效的刷新 Token
      const refreshedTokens = await managerWithProvider.refreshTokens('invalid-refresh-token')
    ...
    it('如果使用访问 Token 而不是刷新 Token 应该返回 null', async () => {
      ...
      const tokenPair = await managerWithProvider.generateTokenPair('user-123', ['user'], ['read'])

      // 使用访问 Token 而不是刷新 Token
      const refreshedTokens = await managerWithProvider.refreshTokens(tokenPair.accessToken)
