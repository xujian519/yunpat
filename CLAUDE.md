...
**项目规模**：~208K 行 Rust + ~196K 行 TypeScript，19 个 Rust crate，16 个 TS 基础设施包，24 个专业 Agent。
...
**19 个 crate**。根 `Cargo.toml` 和 `crates/Cargo.toml` 均为 workspace 定义（内容一致）。需要 Rust 1.88+（使用 `let_chains`）。

| Crate | 行数 | 职责 |
|-------|------|------|
| `tui` | ~170K | **主运行时** — 引擎、工具、LLM 客户端、TUI 渲染、运行时 API、任务管理器（4165 行 main.rs，50+ inline modules） |
| `yunpat-agents` | ~10K | **专利领域 Agent trait 系统**：专利检索、论文搜索、法律库、OCR、文档处理、Flow 编排 |
| `cli` | ~4K | CLI 入口分发器，转发到 `deepseek-tui` |
| `config` | ~2K | 配置加载、profiles、环境变量优先级 |
| `state` | ~1.8K | SQLite 线程/会话持久化 |
| `core` | ~1.7K | 核心运行时边界：Agent 循环、会话管理、Turn 编排、容量流控 |
| `execpolicy` | ~1.5K | 审批/沙箱策略引擎 |
| `yunpat-models` | ~1.2K | 多提供商 ModelProvider 接口（SSE、OpenAI 兼容） |
| `mcp` | ~900 | MCP 客户端 + stdio 传输 |
| `app-server` | ~800 | HTTP/SSE + JSON-RPC 无头 Agent 服务 |
| `hooks` | ~800 | 生命周期钩子（stdout、jsonl、webhook） |
| `secrets` | ~800 | OS keyring API key 存储 |
| `yunpat-mcp-bridge` | ~600 | MCP 桥接：从 Rust 调用 TS/Python Agent |
| `agent` | ~500 | ModelRegistry — 模型/提供商注册表 |
| `tools` | ~500 | 工具调用生命周期、Schema 验证、并行调度器 |
| `protocol` | ~500 | 请求/响应帧和协议类型 |
| `yunpat-router` | ~400 | 意图路由和命令分发 |
| `yunpat-patent-tui` | — | 专利 TUI 模块（从 tui 提取） |

**关键**：`tui` 是实际主运行时（~170K 行，包含原始 deepseek-coder 代码库），其他 crate 正在逐步提取但尚未完全独立。

### TypeScript 工作区（packages/）

**pnpm workspace，16 个基础设施包 + 24 个专业 Agent 包**。

#### 基础设施包

| 包名 | 行数 | 职责 |
|------|------|------|
| `@yunpat/core` | ~55K | 智能体框架核心：Agent 抽象、事件总线、生命周期、LLM 集成、DB、内存、规划 |
| `@yunpat/orchestrator` | ~6.6K | 专利 Agent 中央调度系统，组合 24 个专业 Agent |
| `@yunpat/document-tools` | ~5.3K | 文档解析工具集 — PDF、DOCX、Excel、图片、音频 |
| `@yunpat/builtin-tools` | ~4K | 内置工具集 — 文件操作、搜索、网络请求 |
| `@yunpat/mcp-server` | ~3.3K | MCP 协议服务端（bin: `yunpat-mcp`），向 Claude Desktop 暴露专利工具 |
| `@yunpat/skills` | ~2.8K | 模块化提示词管理系统 |
| `@yunpat/patent-tools` | ~2.8K | 专利专用工具：权利要求生成、质量评估、审查意见答复 |
| `@yunpat/unified-knowledge-graph` | ~2.7K | 统一知识图谱（OpenClaw + YunPat + Athena） |
| `@yunpat/orchestrator-adapter` | ~1.6K | 连接 Rust Gateway 和 Node.js 业务层（Anthropic SDK + Express） |
| `@yunpat/patent-database` | ~1.2K | 专利数据库访问层（Drizzle ORM，支持 patent_db 7500 万 CN 专利 + Google Patents） |
| `@yunpat/patent-knowledge` | ~750 | 专利知识库访问（Obsidian 集成） |
| `@yunpat/patent-core` | ~560 | Rust CLI 桥接和 TypeScript 降级层 |
| `@yunpat/grpc-server` | ~430 | gRPC Server（TypeScript 实现） |
| `@yunpat/image-tools` | ~320 | 图像识别工具 — 化学结构、数学公式 |
| `@yunpat/execpolicy-hook` | ~310 | 执行策略 Hook — 参数级安全检查和审批门控 |
| `@yunpat/patent-prompts` | ~270 | 专利提示词模板管理器（分步加载和缓存） |

#### 24 个专业 Agent

| Agent | 行数 | 职责 |
|-------|------|------|
| `patent-responder` | ~6.7K | OA 审查意见答复与策略生成 |
| `patent-manager` | ~3.3K | 专利全生命周期管理与监控 |
| `invention` | ~2.1K | 专利交底书分析与结构化理解 |
| `search` | ~1.8K | 专利检索策略生成与执行 |
| `specification-drafter` | ~1.7K | 分章节撰写专利说明书 |
| `quality-checker` | ~1.5K | 专利申请质量评估 |
| `claim-generator` | ~1.4K | 基于发明理解和检索分析撰写权利要求 |
| `patent-analyzer` | ~1.4K | 专利文献深度分析 |
| `analysis` | ~1.3K | 现有技术深度分析、对比分析、交底书再分析 |
| `quality` | ~1K | 权利要求/说明书/术语一致性检查 |
| `tech-unit` | ~920 | 专利技术特征划分与最小技术单元识别 |
| `writer` | ~750 | 集成知识库和分步加载提示词模板的专利撰写 |
...
| `legal-qa` | ~500 | 基于法律世界模型的三库联动问答 |
| `format-converter` | ~470 | Markdown/结构化内容转 DOCX |
...
`knowledge-base/` — 本地知识库（不提交 git）：
- `法律法规/`、`审查指南/`、`复审无效/` — 法律法规文件
- `专利判决/`、`专利侵权/`、`专利实务/` — 实务案例
- `All-Concepts.md`、`Concept-Hierarchy.md` — 知识产权概念体系
