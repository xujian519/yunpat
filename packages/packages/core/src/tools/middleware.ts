...
export class LoggingMiddleware implements Middleware {
  ...
  async before(ctx: ToolExecutionContext): Promise<void> {
    ...
    this.logger.debug(`Calling ${ctx.tool.metadata.name}`, {
      input,
  ...
  async after(ctx: ToolExecutionContext, result: unknown): Promise<void> {
    ...
    this.logger.info(`${ctx.tool.metadata.name} completed in ${duration}ms`, {
      output,
      duration,
  ...
  async onError(ctx: ToolExecutionContext, error: Error): Promise<void> {
    const duration = Date.now() - ctx.startTime
    this.logger.error(`${ctx.tool.metadata.name} failed after ${duration}ms`, {
      error: error.message,
      stack: error.stack,
    })
  }
...
export class PermissionMiddleware implements Middleware {
  ...
  constructor(
    ...
    this.logger = createLogger('PermissionMiddleware')
    // 默认权限检查器：授予所有权限
  ...
  async onError(ctx: ToolExecutionContext, error: Error): Promise<void> {
    if (error.message.includes('Permission denied')) {
      this.logger.warn(`Access denied to tool ${ctx.tool.metadata.name}`, {
        userId: ctx.context.userId,
...
export class CacheMiddleware implements Middleware {
  ...
  constructor() {
    this.logger = createLogger('CacheMiddleware')
  }

  async before(ctx: ToolExecutionContext): Promise<void> {
    // 只缓存只读工具（优先使用 isReadOnly 方法，回退到 metadata.isConcurrencySafe）
    const tool = ctx.tool
    ...
    if (cached && cached.expiry > Date.now()) {
      ...
      this.logger.debug(`Cache hit for tool ${ctx.tool.metadata.name}`)
    } else {
      this.stats.misses++
    }
  }
  ...
  async after(ctx: ToolExecutionContext, result: unknown): Promise<void> {
    ...
    this.logger.debug(`Cached result for ${ctx.tool.metadata.name} (TTL: ${ttl}ms)`)
  }
...
export class RateLimitMiddleware implements Middleware {
  ...
  constructor() {
    this.logger = createLogger('RateLimitMiddleware')
  }
  ...
  async onError(ctx: ToolExecutionContext, error: Error): Promise<void> {
    if (error.message.includes('Rate limit exceeded')) {
      this.logger.warn(`Limit exceeded for tool ${ctx.tool.metadata.name}`, {
        userId: ctx.context.userId,
      })
    }
  }
...
export class TracingMiddleware implements Middleware {
  ...
  async before(ctx: ToolExecutionContext): Promise<void> {
    ...
    this.logger.debug(`Starting ${ctx.tool.metadata.name}`, {
      traceId,
      userId: ctx.context.userId,
      sessionId: ctx.context.sessionId,
  ...
  async after(ctx: ToolExecutionContext, _result: unknown): Promise<void> {
    ...
    this.logger.debug(`Completed ${ctx.tool.metadata.name} in ${duration}ms`, {
      traceId,
      duration,
      cached: ctx.cached || false,
    })
  ...
  async onError(ctx: ToolExecutionContext, error: Error): Promise<void> {
    ...
    this.logger.error(`Failed ${ctx.tool.metadata.name} after ${duration}ms`, {
