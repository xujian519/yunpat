...
**项目规模**：~208K 行 Rust + ~196K 行 TypeScript，18 个 Rust crate，16 个 TS 基础设施包，24 个专业 Agent。
...
**18 个 crate**。根 `Cargo.toml` 和 `crates/Cargo.toml` 均为 workspace 定义（内容一致）。需要 Rust 1.88+（使用 `let_chains`）。
