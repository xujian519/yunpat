...
async function testCodeQualityFixes(): Promise<TestResult[]> {
  ...
  try {
    ...
    const hasEnvVarSupport =
      configContent.includes('process.env.BGE_API_KEY') ||
  ...
  try {
    ...
    const hasVectorType =
      vectorStoreContent.includes('vector(1024)') &&
  ...
  try {
    ...
    if (!existsSync(graphStorePath)) {
      ...
      const hasCorrectImport =
        graphStoreContent.includes('import { eq, and, or, sql') ||
  ...
  try {
    ...
    const hasBatchComment =
      vectorStoreContent.includes('批量插入') || vectorStoreContent.includes('真批量')
    const hasTrueBatch = hasInsert && hasValues && hasBatchComment

    console.log(
      `  🔍 调试: hasInsert = ${hasInsert}, hasValues = ${hasValues}, hasBatchComment = ${hasBatchComment}`
    )
  ...
  try {
    ...
    const hasLRU =
      bgeContent.includes('this.cache.size >= this.cacheMaxSize') &&
  ...
  try {
    ...
    const hasCompressImpl =
      memoryStoreContent.includes('async compress(') &&
  ...
  try {
    const tokenWindowPath = join(
      process.cwd(),
      'packages/core/src/memory/short-term/TokenWindow.ts'
    )
    const tokenWindowContent = readFileSync(tokenWindowPath, 'utf-8')

    const hasImprovedEstimation =
      tokenWindowContent.includes('/ 1.3') && tokenWindowContent.includes('englishWords')
