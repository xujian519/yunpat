...
import { DependencyType } from '../planning/types.js';
...
export class IncrementalPlanner {
    ...
    dependencyAnalyzer;
    taskIdCounter = 0;
    currentCriticalPath = null;
    constructor(config = {}, taskDecomposer) {
        this.config = {
     ...
     * 应用增量调整
     */
    async applyAdjustment(plan, adjustment) {
        // 创建计划的深拷贝
     ...
     * 生成增量调整
     */
    async generateIncrementalAdjustment(context, strategy) {
        const modifications = [];
        // 根据策略类型生成相应的修改
     ...
     * 生成重试修改
     */
    generateRetryModifications(context) {
        const modifications = [];
     ...
     * 生成跳过修改
     */
    generateSkipModifications(context) {
        const modifications = [];
        // 找出被阻塞的任务
     ...
     * 生成重排序修改
     */
    generateReorderModifications(context) {
        const modifications = [];
        // 找出未完成的任务
     ...
     * 生成分解修改
     */
    generateDecomposeModifications(context) {
        const modifications = [];
        // 找出复杂的任务（时间或token超过阈值）
        const complexGoals = context.originalPlan.subGoals.filter((g) => {
            const avgDuration = context.originalPlan.subGoals.reduce((sum, g) => sum + g.estimatedDuration, 0) /
                context.originalPlan.subGoals.length;
            const avgTokens = context.originalPlan.subGoals.reduce((sum, g) => sum + g.estimatedTokens, 0) /
                context.originalPlan.subGoals.length;
     ...
     * 生成适应修改
     */
    generateAdaptModifications(context) {
        const modifications = [];
        // 根据资源使用情况调整估算
     ...
     * 应用单个修改
     */
    async applyModification(plan, modification) {
            ...
            case 'modify':
                ...
                if (modification.changes) {
                    ...
                    if (modification.changes.newPriority !== undefined) {
                        goal.priority = modification.changes.newPriority;
                    }
                    if (modification.changes.newStatus !== undefined) {
                        goal.status = modification.changes.newStatus;
                    }
     ...
     * 验证调整
     */
    async validateAdjustment(plan, adjustment) {
        // 检查修改数量
        if (adjustment.modifications.length > this.config.maxModifications) {
            console.warn(`[IncrementalPlanner] 修改数量超过限制: ${adjustment.modifications.length} > ${this.config.maxModifications}`);
            return false;
        }
        // 检查预期改进
        if (adjustment.estimatedImprovement < this.config.minImprovementThreshold) {
            console.warn(`[IncrementalPlanner] 预期改进低于阈值: ${adjustment.estimatedImprovement} < ${this.config.minImprovementThreshold}`);
            return false;
        }
        // 检查是否保护关键路径
     ...
     * @returns 更新后的计划和影响评估
     */
    async addTask(plan, task, dependencies) {
        // 1. 验证任务合法性
     ...
     * 计算最晚开始时间
     */
    calculateLatestStart(plan, earliestStart) {
     ...
     * 计算路径时长
     */
    calculatePathDuration(plan, taskIds) {
        let totalDuration = 0;
        const { dependencies } = plan;
     ...
     * 识别瓶颈任务
     */
    identifyBottlenecks(plan, criticalTasks) {
        const { dependencies } = plan;
     ...
     * @returns 影响评估报告
     */
    assessImpact(task, newCriticalPath, oldPlan) {
     ...
     * 检查是否影响关键路径
     */
    affectsCriticalPath(task, oldCriticalPath, newCriticalPath) {
        // 检查任务是否在新的关键路径上
     ...
     * 检查资源冲突
     */
    checkResourceConflicts(task, plan) {
        const conflicts = [];
        // 收集任务需要的资源
     ...
     * 生成建议措施
     */
    generateRecommendations(impactLevel, delay) {
