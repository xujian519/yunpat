...
export abstract class Agent<
  TInput = any, TOutput = any, TPlan = any, TResult = any, TReflection = any
> {
...
async function runAgent(input: string, prompt: string): Promise<string> {
  const response = await llm.chat([{role:'system',content:prompt},{role:'user',content:input}]);
...
```typescript
export abstract class Agent<TInput = any, TOutput = any, TPlan = any, TResult = any, TReflection = any> {
