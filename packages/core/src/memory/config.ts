 ...
 * 获取 BGE-M3 服务地址
 */
export function getBGEBaseUrl(): string {
 ...
 * 从环境变量加载记忆层配置
 */
export function loadMemoryConfig(userConfig?: MemoryConfig): MemoryConfig {
  return {
 ...
 * 验证配置
 */
export function validateMemoryConfig(config: MemoryConfig): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
