...
export class SAOExtractor {
  ...
  private extractHeuristic(sentence: string): SAOTriplet | null {
    ...
    const subjectMatch = sentence.match(
      new RegExp(`([^\s,，]+?(?:${TECH_COMPONENT_SUFFIXES.join('|')}))`, 'g')
    )
    ...
    const verbMatch = sentence.match(
      /(?:通过|利用|使用|进行|执行|实施|完成|实现|处理|计算|分析|检测|识别|提取|生成|创建|构建|训练|预测|评估|比较|组合|整合|分割|合并|排序|过滤|搜索|匹配|调整|修改|更新|加载|保存|删除|添加|插入|连接|打开|关闭|启动|停止|初始化|配置|设置|定义|指定|分配|存储|发送|传输|优化|转换|映射)/
    )
    ...
    const startIndex =
      Math.max(subjectIndex, verbIndex) + Math.max(subjectMatch[0].length, verbMatch[0].length)
