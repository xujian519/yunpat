...
export class FileSystemCheckpointStore {
   ...
   * 加载检查点
   */
  async load(checkpointId: string, executionId?: string): Promise<Checkpoint> {
   ...
   * 删除检查点
   */
  async delete(checkpointId: string, executionId?: string): Promise<void> {
    const execId = executionId ?? this.extractExecutionId(checkpointId)
   ...
   * 从检查点ID提取执行ID
   */
  private extractExecutionId(checkpointId: string): string | null {
