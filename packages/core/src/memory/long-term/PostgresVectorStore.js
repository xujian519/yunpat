...
export class PostgresVectorStore {
    ...
    async upsert(memory) {
        ...
        const embeddingString = typeof memory.embedding === 'string' ? memory.embedding : JSON.stringify(memory.embedding);
        if (memory.id) {
            // 更新
            await this.db
                .update(memories)
    ...
    async search(queryEmbedding, topK = 10, filter) {
        ...
        try {
            ...
            if (queryEmbedding.length !== this.vectorDimension) {
                throw new Error(`向量维度不匹配：期望 ${this.vectorDimension}，实际 ${queryEmbedding.length}`);
            }
            // 构建查询条件
            const conditions = [];
