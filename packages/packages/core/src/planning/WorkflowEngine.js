...
export class WorkflowEngine {
  ...
  MAX_ACTIVE_WORKFLOWS = 100 // 最大活动工作流数量
  constructor(config) {
    this.config = config
  }
  /**
  ...
  cleanupOldWorkflows() {
    ...
    // 获取所有执行并按开始时间排序
    const executions = Array.from(this.activeWorkflows.values()).sort(
  ...
  async runExecution(execution) {
    ...
    try {
      ...
      console.log(
        `[WorkflowEngine] 工作流执行成功: ${workflow.name} (耗时: ${result.totalDuration}ms)`
      )
      return result
    } catch (error) {
      const endTime = new Date()
      ...
      if (errorMessage.startsWith('WORKFLOW_PAUSED:')) {
        console.log(
          `[WorkflowEngine] 工作流已暂停: ${workflow.name} (执行ID: ${execution.executionId})`
        )
        return {
          workflowId: workflow.id,
          executionId: execution.executionId,
   ...
   * 执行单个步骤
   */
  async executeStep(step, execution, initialInput) {
    const startTime = new Date()
   ...
   */
  prepareStepInput(step, execution, initialInput) {
  ...
  async saveCheckpoint(execution) {
  ...
  async resume(executionId) {
    ...
    if (!execution && this.config.checkpointManager) {
      try {
        ...
        if (checkpoints.length > 0) {
          ...
          const context = latest.contextSnapshot
          ...
          const workflowId =
            typeof context.workflowId === 'string' ? context.workflowId : executionId
          const currentStepIndex =
            typeof context.currentStepIndex === 'number' ? context.currentStepIndex : 0
    ...
    this.activeWorkflows.set(executionId, execution)
    console.log(
      `[WorkflowEngine] 恢复工作流执行: ${executionId} (从步骤 ${execution.currentStepIndex})`
    )
    return this.runExecution(execution)
  }
