...
describe('OAuthManager', () => {
  ...
  describe('refreshToken', () => {
    ...
    it('应该处理刷新失败', async () => {
      ...
      await expect(
        oauthManager.refreshToken('google', 'invalid-refresh-token')
      ).rejects.toThrow();
    });
  });
  ...
  describe('cleanupExpiredStates', () => {
    it('应该清理过期的 State', async () => {
      ...
      // 生成几个 state
      await shortLivedManager.generateAuthorizationUrl(
        'google',
        'http://localhost:3000/callback'
      );
      await shortLivedManager.generateAuthorizationUrl(
...
describe('GoogleOAuth', () => {
  ...
  it('应该返回正确的端点', () => {
    ...
    expect(provider.getAuthorizationEndpoint()).toBe(
      'https://accounts.google.com/o/oauth2/v2/auth'
    );
    expect(provider.getTokenEndpoint()).toBe(
      'https://oauth2.googleapis.com/token'
    );
    expect(provider.getUserInfoEndpoint()).toBe(
