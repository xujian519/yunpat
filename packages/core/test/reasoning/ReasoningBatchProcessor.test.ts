...
describe('ReasoningBatchProcessor', () => {
  ...
  it('应该使用缓存', async () => {
    const cache = {
      ...
      store: async () => {},
      getStats: () => ({ totalEntries: 0, hits: 0, misses: 0, hitRate: 0, tokensSaved: 0, totalTokens: 0, avgSimilarity: 0 }),
    } as any;

    const processor = new ReasoningBatchProcessor({ useCache: true }, cache);
    const results = await processor.processBatch(['a'], async (x) => x, (x) => x);
