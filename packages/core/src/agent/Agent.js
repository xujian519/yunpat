...
export class Agent {
    ...
    async executeInternal(input) {
        ...
        try {
            ...
            while (shouldContinue && iterations < this.maxIterations) {
                ...
                iterations++;
                await this.saveCheckpointIfEnabled(executionId, ++iteration, context, `act-iter${iterations}`, { result, iteration: iterations });
                if (this.reflect) {
                    context.currentStage = LifecycleStage.REFLECT;
                    const reflection = await this.reflect(result, context);
                    await this.saveCheckpointIfEnabled(executionId, iteration, context, `reflect-iter${iterations}`, { reflection });
    ...
    async saveCheckpointIfEnabled(executionId, iteration, context, stageName, additionalData) {
        ...
        try {
            ...
            await this.checkpointManager.saveCheckpoint(this.name, executionId, iteration, memorySnapshot, contextSnapshot, stateSnapshot, [stageName], `阶段: ${stageName}`);
