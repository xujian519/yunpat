...
while IFS=$'\t' read -r kind name value; do
  case "${kind}" in
    version)
      ...
      ;;
    crate)
      workspace_yunpat_packages+=("${name}")
...
for pkg in sorted(workspace_packages, key=lambda item: item["name"]):
    if not pkg["name"].startswith("yunpat-"):
        continue
...
missing_packages=()
for workspace_package in "${workspace_yunpat_packages[@]}"; do
  found=0
...
package_has_workspace_deps() {
  ...
  local index
  for ((index = 0; index < ${#workspace_yunpat_packages[@]}; index += 1)); do
    if [[ "${workspace_yunpat_packages[$index]}" == "${package_name}" ]]; then
