...
mod tests {
    ...
    fn existing_skill_dirs_orders_globals_agents_then_claude_then_deepseek() {
        ...
        let yunpat_global = tmpdir.path().join(".deepseek").join("skills");
        std::fs::create_dir_all(&agents_global).unwrap();
        std::fs::create_dir_all(&claude_global).unwrap();
        std::fs::create_dir_all(&yunpat_global).unwrap();
        ...
        assert_eq!(dirs, vec![agents_global, claude_global, yunpat_global]);
    }

    #[test]
    fn existing_skill_dirs_keeps_agents_global_before_yunpat_global() {
        let tmpdir = TempDir::new().unwrap();
        let agents_global = tmpdir.path().join(".agents").join("skills");
        let yunpat_global = tmpdir.path().join(".deepseek").join("skills");
        ...
        assert_eq!(dirs, vec![agents_global, yunpat_global]);
    }
