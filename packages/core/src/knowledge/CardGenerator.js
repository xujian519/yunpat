...
export class CardGenerator {
    ...
    async generateFromPage(pagePath, pageContent, pageTitle, concept, domain) {
    ...
    async generateFromConcept(concept, domain, pages) {
        const allCards = [];
        for (const page of pages) {
            try {
                const cards = await this.generateFromPage(page.path, page.content, page.title, concept, domain);
                allCards.push(...cards);
            }
            catch (error) {
                console.error(`生成卡片失败 (${page.path}): ${error}`);
            }
            if (allCards.length >= this.maxCardsPerPage * pages.length)
                break;
        }
        return this.deduplicate(allCards);
    }
    async assessQuality(card) {
        const prompt = QUALITY_ASSESS_PROMPT.replace('{question}', card.question).replace('{content}', card.content);
        try {
            const response = await this.llm.chat({
                messages: [
    ...
    parseGenerationResponse(content, sourcePage, concept, domain) {
    ...
    extractJSON(text) {
        ...
        if (codeBlockMatch) {
            try {
                ...
                console.warn('JSON 代码块解析失败:', e.message);
            }
        }
        // 尝试直接解析
        try {
