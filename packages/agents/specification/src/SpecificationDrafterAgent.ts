...
export class SpecificationDrafterAgent extends Agent {
  ...
  protected async plan(
    ...
    if (input.claims) {
      console.log(
        `   权利要求: ${input.claims.independentClaims.length} 项独立, ${input.claims.dependentClaims.length} 项从属`
      )
    }
  ...
  protected async act(plan: DraftingPlan, context: ExecutionContext): Promise<PatentSpecification> {
    console.log('\n✍️ [说明书撰写] 步骤2: 撰写阶段')
    ...
    console.log(
      `      ✅ 问题:${inventionContent.technicalProblem.length}字 方案:${inventionContent.technicalSolution.length}字 效果:${inventionContent.beneficialEffects.length}字`
    )
    ...
    console.log(
      `   总字数: ${technicalField.length + backgroundArt.length + inventionContent.technicalSolution.length + inventionContent.beneficialEffects.length + drawingsDescription.length + detailedDescription.length}`
    )
