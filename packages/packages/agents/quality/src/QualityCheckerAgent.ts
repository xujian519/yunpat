...
export class QualityCheckerAgent extends ProfessionalAgent {
  protected async plan(
    ...
    if (!normalizedInput.claims?.independentClaims?.length) {
      throw new Error('权利要求不能为空')
    }
    if (!normalizedInput.specification?.technicalField?.trim()) {
      throw new Error('说明书不能为空')
    }

    console.log('\n🔍 [质量检查] 步骤1: 规划阶段')
    console.log(`   独立权利要求: ${normalizedInput.claims.independentClaims.length} 项`)
    console.log(`   说明书章节: 完整`)

    return { input: normalizedInput }
  }

  private normalizeInput(input: any): QualityCheckerInput {
    // 转换 claims 字段
    const claims = input.claims || {}
    const independentClaims = claims.independentClaims || claims.independent_claims || []
    const dependentClaims = claims.dependentClaims || claims.dependent_claims || []

    // 转换 specification 字段
    const spec = input.specification || {}
    const normalizedSpec = {
      technicalField: spec.technicalField || spec.technical_field?.content || '',
      backgroundArt: spec.backgroundArt || spec.background_art?.content || '',
      inventionContent: spec.inventionContent || {
        technicalProblem: spec.invention_content?.technical_problem || '',
        technicalSolution: spec.invention_content?.technical_solution || '',
        beneficialEffects: spec.invention_content?.beneficial_effects || '',
      },
      drawingsDescription: spec.drawingsDescription || spec.drawings_description || '',
      detailedDescription: spec.detailedDescription || spec.detailed_description?.content || '',
      abstract: spec.abstract || spec.abstract?.content || '',
    }

    // 转换 inventionUnderstanding 字段
    const iu = input.inventionUnderstanding || input.invention_understanding
    const normalizedInvention = iu
      ? {
          technicalProblem: iu.technicalProblem || iu.technical_problem || '',
          keyFeatures: iu.keyFeatures || iu.key_features || [],
          technicalSolution: iu.technicalSolution || iu.technical_solution || '',
        }
      : undefined

    return {
      claims: {
        independentClaims: independentClaims.map((c: any) => ({
          claimNumber: c.claimNumber || c.claim_number || 0,
          fullText: c.fullText || c.full_text || '',
          claimType: c.claimType || c.claim_type || 'independent',
          essentialFeatures: c.essentialFeatures || c.essential_features || [],
        })),
        dependentClaims: dependentClaims.map((c: any) => ({
          claimNumber: c.claimNumber || c.claim_number || 0,
          content: c.content || c.full_text || '',
          parentClaim: c.parentClaim || c.parent_claim || 0,
          additionalFeatures: c.additionalFeatures || c.additional_features || [],
        })),
      },
      specification: normalizedSpec,
      inventionUnderstanding: normalizedInvention,
    }
  }
...
{\n  "score": 85,\n  "disclosure": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  },\n  "term_consistency": {\n    "status": "pass | warning | fail",\n    "inconsistent_terms": [{ "term": "术语", "occurrences": ["章节1", "章节2"] }]\n  },\n  "completeness": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  }\n}`
  ...
  private async checkFormal(
    ...
    // CLAIM_005: 必要技术特征完整性检查
    const inventionData = input.inventionUnderstanding
    if (inventionData?.keyFeatures?.length) {
      // 检查每个必要特征是否被至少一个独立权利要求覆盖
      for (const feature of inventionData.keyFeatures) {
        const featureTerms = feature.match(/[一-龥a-zA-Z]{2,}/g) || []
        const coveredByClaims: number[] = []

        for (const claim of input.claims.independentClaims) {
          const isCovered =
            claim.fullText.includes(feature) ||
            (featureTerms.length >= 2 &&
              featureTerms.filter((t: string) => claim.fullText.includes(t)).length /
                featureTerms.length >=
                0.6)
          if (isCovered) {
            coveredByClaims.push(claim.claimNumber)
          }
        }

        if (coveredByClaims.length === 0) {
          errors.push({
            type: '必要技术特征缺失',
            location: `权利要求${input.claims.independentClaims.map((c) => c.claimNumber).join('、')}`,
            description: `所有独立权利要求均缺少必要技术特征"${feature}"（实施细则第20条第1款）`,
            severity: 'error',
          })
        }
      }
