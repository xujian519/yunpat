...
const annotatedDatasets: AnnotatedData[] = [
  ...
  },
  {
    ...
    expectedRelations: [{ from: '本专利', to: 'CN202110123456.7', type: 'priority' }],
  },
]
...
describe('实体抽取准确率测试', () => {
  ...
  it('应该正确识别IPC分类号（F1 > 0.90）', async () => {
    ...
    expect(metrics.f1).toBeGreaterThan(0.9)
  })
...
describe('关系抽取准确率测试', () => {
  ...
  it('应该正确识别引用关系（F1 > 0.75）', async () => {
    ...
    const expected = [{ from: '本专利', to: 'CN202210987654.3', type: 'cites' }]
...
describe('实体归一化准确率测试', () => {
  ...
  it('实体归一化准确率应该 > 90%', async () => {
    ...
    for (const testCase of testCases) {
      const entities = await extractor.extractEntities(testCase.input, testCase.type as any)
