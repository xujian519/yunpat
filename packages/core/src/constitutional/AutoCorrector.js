...
export class AutoCorrector {
     ...
     * 执行自动纠正
     */
    async correct(content, violations) {
     ...
     * 过滤可纠正的违规
     */
    filterCorrectableViolations(violations) {
     ...
     * 基于规则的纠正
     */
    async correctWithRules(content, violations) {
     ...
     * 应用单个规则纠正
     */
    async applyRuleCorrection(content, violation) {
        // 调试日志
        ...
        if (suggestedCorrection && suggestedCorrection.length > 0 && suggestedCorrection.length < 10) {
            ...
            const correctedContent = content.substring(0, location.start) + correctedText + content.substring(location.end);
        ...
        if (correction !== null) {
            ...
            const correctedContent = content.substring(0, location.start) + correction + content.substring(location.end);
     ...
     * 应用通用纠正规则
     */
    applyGenericCorrection(violation, originalText) {
        // 清楚性原则：替换模糊词汇
     ...
     * 基于LLM的纠正
     */
    async correctWithLLM(content, violations) {
        ...
        for (const violation of sortedViolations) {
            ...
            if (correction) {
                ...
                const newCorrectedContent = correctedContent.substring(0, violation.location.start) +
     ...
     * 应用单个LLM纠正
     */
    async applyLLMCorrection(content, violation) {
        try {
            ...
            if (correctedText && correctedText !== violation.location.text) {
                ...
                const _correctedContent = content.substring(0, violation.location.start) +
                    correctedText +
     ...
     * 构建纠正提示词
     */
    buildCorrectionPrompt(content, violation) {
...
**纠正后的文本**:`;
     ...
     * 混合纠正策略
     */
    async correctWithHybrid(content, violations) {
        const appliedCorrections = [];
        let correctedContent = content;
        // 分为两类违规：简单的用规则，复杂的用LLM
        const ruleBasedViolations = [];
        const llmBasedViolations = [];
     ...
     * 验证纠正结果
     */
    async verifyCorrection(originalContent, correctedContent, violations) {
        ...
        if (lengthRatio < 0.5 || lengthRatio > 1.5) {
            console.warn(`[AutoCorrector verifyCorrection] 纠正后文本长度变化过大: ${(lengthRatio * 100).toFixed(1)}%`);
            return false;
        }
        ...
        if (essentialFeatures) {
            ...
            if (correctedFeatureCount < featureCount * 0.8) {
                console.warn(`[AutoCorrector verifyCorrection] 纠正后可能丢失技术特征: ${featureCount} -> ${correctedFeatureCount}`);
                return false;
            }
        }
