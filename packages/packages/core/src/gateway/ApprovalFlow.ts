...
import { WebSocketApprovalServer } from './websocket/WebSocketApprovalServer.js'
import type { ApprovalRequest } from './Gateway.js'
import type { TemplateManager } from '../prompts/PromptTemplate.js'
...
export interface ApprovalFlowConfig {
  ...
  /** WebSocket 审批配置（WebSocket 模式时需要） */
  websocketConfig?: {
    port: number
    host?: string
    apiKey?: string
...
export class ApprovalFlow {
  ...
  private wsServer?: WebSocketApprovalServer
  private templateManager?: TemplateManager

  constructor(config: ApprovalFlowConfig, eventBus?: EventBus, templateManager?: TemplateManager) {
    this.config = config
    ...
    if (config.mode === ApprovalMode.WEBSOCKET && config.websocketConfig) {
      this.wsServer = new WebSocketApprovalServer(config.websocketConfig)
    }
  ...
  async learnFromFeedback(feedback: UserFeedback): Promise<void> {
    ...
    if (this.templateManager) {
      try {
        const correctionPatterns = this.analyzeFeedbackPatterns()

        for (const templateName of this.templateManager.listTemplates()) {
          const template = this.templateManager.getTemplate(templateName)
          if (!template) continue

          const metadata = template.getMetadata()
          if (
            metadata.tags?.some((tag) => feedback.type === 'reject' && tag.includes('approval'))
          ) {
            const currentVersion = template.getVersion()
            template.updateVersion(
              `${currentVersion.split('.').slice(0, 2).join('.')}.${parseInt(currentVersion.split('.').pop() || '0') + 1}`
            )
          }
        }

        if (correctionPatterns.length > 0) {
          console.log(`[反馈学习] 检测到 ${correctionPatterns.length} 个修正模式`)
        }
      } catch (err) {
        console.error('[反馈学习] 模板更新失败:', err)
      }
    }
  ...
  private analyzeFeedbackPatterns(): Array<{ field: string; frequency: number }> {
    const fieldCorrections = new Map<string, number>()

    for (const [, feedbacks] of this.feedbackHistory) {
      for (const fb of feedbacks) {
        if (fb.type === 'correct' && fb.corrections) {
          for (const field of Object.keys(fb.corrections)) {
            fieldCorrections.set(field, (fieldCorrections.get(field) || 0) + 1)
          }
        }
      }
    }

    return Array.from(fieldCorrections.entries())
      .filter(([, freq]) => freq >= 2)
  ...
  private async websocketApproval(
    ...
    const wsResponse = await this.wsServer.requestApproval(
      {
        ...
        context: {
          goal: context.executionId,
          reasoning: 'Pending approval via WebSocket',
          alternatives: [],
        },
        level: 'info',
        timeout,
    ...
    const response: ApprovalResponse = {
      approvalId: wsResponse.approvalId,
      approved: wsResponse.approved,
      timestamp: new Date(),
    }

    if (wsResponse.feedback) {
      response.feedback = {
        type: wsResponse.feedback.type,
        content: wsResponse.feedback.content,
        corrections: wsResponse.feedback.corrections,
        supplements: wsResponse.feedback.supplements,
        rejectionReason: wsResponse.feedback.rejectionReason,
        timestamp: new Date(),
      }
    }
   ...
   * 收集WebSocket反馈
   */
  private async collectWebSocketFeedback(approvalId: string): Promise<UserFeedback> {
    const pendingApproval = this.wsServer ? await this.getWebSocketApproval(approvalId) : null

    if (pendingApproval?.feedback) {
      return {
        type: pendingApproval.feedback.type,
        content: pendingApproval.feedback.content,
        corrections: pendingApproval.feedback.corrections,
        supplements: pendingApproval.feedback.supplements,
        rejectionReason: pendingApproval.feedback.rejectionReason,
        timestamp: new Date(),
      }
    }
  ...
  private async getWebSocketApproval(_approvalId: string): Promise<ApprovalResponse | null> {
    return null
  }
