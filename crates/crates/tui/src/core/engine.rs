...
pub struct Engine {
    config: EngineConfig,
    yunpat_client: Option<Arc<dyn LlmClient>>,
    yunpat_client_error: Option<String>,
...
impl Engine {
    ...
    pub fn new(config: EngineConfig, api_config: &Config) -> (Self, EngineHandle) {
        ...
        let (yunpat_client, yunpat_client_error) = match DeepSeekClient::new(api_config) {
        ...
        let seam_manager = yunpat_client.as_ref().map(|main_client| {
        ...
        let tool_registry = match mode {
            AppMode::Agent | AppMode::Yolo => {
                if self.config.features.enabled(Feature::Subagents) {
                    let runtime = if let Some(client) = self.yunpat_client.clone() {
    ...
    async fn handle_rlm(
        ...
        let Some(ref client) = self.yunpat_client else {
            let err = self
                .yunpat_client_error
                .as_deref()
                .map(|s| s.to_string())
    ...
    /// assistant message. Called from `handle_yunpat_turn` before each API
