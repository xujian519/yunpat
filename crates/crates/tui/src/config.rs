...
impl Config {
    ...
    pub(crate) fn provider_config_for(&self, provider: ApiProvider) -> Option<&ProviderConfig> {
        ...
        Some(match provider {
            ...
            ApiProvider::DeepseekCN => &providers.yunpat_cn,
    ...
    #[must_use]
    pub fn yunpat_base_url(&self) -> String {
        let provider = self.api_provider();
    ...
    pub fn yunpat_api_key(&self) -> Result<String> {
        let provider = self.api_provider();
        ...
        if let Some(value) = yunpat_secrets::env_for(slot)
            && !value.trim().is_empty()
        {
            return Ok(value);
        }
...
fn apply_env_overrides(config: &mut Config) {
    ...
    let capacity = config.capacity.get_or_insert(CapacityConfig {
        ...
        profile_window: None,
        yunpat_v3_2_chat_prior: None,
        yunpat_v3_2_reasoner_prior: None,
        yunpat_v4_pro_prior: None,
        yunpat_v4_flash_prior: None,
    ...
    if config.capacity.as_ref().is_some_and(|c| {
        c.enabled.is_none()
            ...
            && c.yunpat_v3_2_chat_prior.is_none()
            && c.yunpat_v3_2_reasoner_prior.is_none()
            && c.yunpat_v4_pro_prior.is_none()
            && c.yunpat_v4_flash_prior.is_none()
...
fn normalize_model_config(config: &mut Config) {
    ...
    if let Some(providers) = config.providers.as_mut() {
        ...
        if let Some(model) = providers.yunpat_cn.model.as_deref()
            && let Some(normalized) = normalize_model_for_provider(ApiProvider::DeepseekCN, model)
        {
            providers.yunpat_cn.model = Some(normalized);
        }
...
fn normalize_base_url(base: &str) -> String {
    ...
    let yunpat_domains = ["api.deepseek.com", "api.deepseeki.com"];
    if yunpat_domains.iter().any(|domain| trimmed.contains(domain)) {
...
fn merge_providers(
    ...
    match (base, override_cfg) {
        ...
        (Some(base), Some(override_cfg)) => Some(ProvidersConfig {
            ...
            yunpat_cn: merge_provider_config(base.yunpat_cn, override_cfg.yunpat_cn),
...
pub fn save_api_key(api_key: &str) -> Result<SavedCredential> {
    ...
    #[cfg(not(test))]
    {
        let secrets = yunpat_secrets::Secrets::auto_detect();
...
/// CLI's purview. `Config::yunpat_api_key`'s explicit-override path
...
mod tests {
    ...
    struct EnvGuard {
        ...
        yunpat_config_path: Option<OsString>,
        ...
        yunpat_default_text_model: Option<OsString>,
    ...
    impl EnvGuard {
        fn new(home: &Path) -> Self {
            ...
            let yunpat_config_prev = env::var_os("DEEPSEEK_CONFIG_PATH");
            let yunpat_provider_prev = env::var_os("DEEPSEEK_PROVIDER");
            ...
            Self {
                ...
                yunpat_config_path: yunpat_config_prev,
                yunpat_provider: yunpat_provider_prev,
                yunpat_api_key: api_key_prev,
                yunpat_base_url: base_url_prev,
                yunpat_http_headers: http_headers_prev,
                yunpat_model: model_prev,
                yunpat_default_text_model: default_text_model_prev,
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                Self::restore_var("DEEPSEEK_CONFIG_PATH", self.yunpat_config_path.take());
                ...
                Self::restore_var(
                    ...
                    self.yunpat_default_text_model.take(),
                );
    ...
    fn has_api_key_detects_in_memory_override_and_env_var() -> Result<()> {
        ...
        // sources used by `Config::yunpat_api_key` (in-memory override,
        // env var, config-file slot).
        let _lock = lock_test_env();
        // Explicit in-memory key wins over every other source per
        // `Config::yunpat_api_key`'s "Path 0" override.
        let cfg = Config {
...
"#,
    ...
    fn yunpat_api_key_prefers_saved_config_over_stale_env() -> Result<()> {
        ...
        assert_eq!(config.yunpat_api_key()?, "fresh-config-key");
        unsafe {
            env::remove_var("DEEPSEEK_API_KEY");
        }
        Ok(())
    ...
    fn apply_env_overrides_does_not_copy_api_key_into_config() -> Result<()> {
        ...
        assert_eq!(config.yunpat_api_key()?, "env-key");
        unsafe {
            env::remove_var("DEEPSEEK_API_KEY");
        }
        Ok(())
    ...
    #[test]
    fn normalize_model_name_rejects_invalid_or_non_yunpat_ids() {
    ...
    #[test]
    fn normalize_model_name_accepts_provider_prefixed_yunpat_ids() {
        assert_eq!(
    ...
    #[test]
    fn validate_accepts_future_yunpat_model_id() -> Result<()> {
        let config = Config {
    ...
    #[test]
    fn yunpat_provider_defaults_to_beta_endpoint() {
        let config = Config::default();
    ...
    #[test]
    fn explicit_yunpat_base_url_overrides_beta_default() {
        let config = Config {
        ...
        assert_eq!(config.api_provider(), ApiProvider::Deepseek);
        assert_eq!(config.yunpat_base_url(), "https://api.deepseek.com");
    }
...
"#,
    ...
    fn nvidia_nim_provider_uses_nim_defaults() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_NVIDIA_NIM_BASE_URL);
        Ok(())
    }
    ...
    #[test]
    fn nvidia_nim_provider_normalizes_yunpat_v4_flash_alias() -> Result<()> {
        let config = Config {
            provider: Some("nvidia-nim".to_string()),
    ...
    fn nvidia_nim_env_overrides_provider_and_credentials() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.yunpat_api_key()?, "nim-env-key");
        assert_eq!(config.default_model(), DEFAULT_NVIDIA_NIM_MODEL);
        Ok(())
    }
    ...
    fn nvidia_nim_env_accepts_short_nim_base_url_alias() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.yunpat_base_url(), "https://short-nim.example/v1");
        Ok(())
    }
    ...
    fn nvidia_nim_env_accepts_facade_base_url_forwarding() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.yunpat_base_url(), "https://forwarded-nim.example/v1");
        Ok(())
    }
    ...
    fn openrouter_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_OPENROUTER_BASE_URL);
        Ok(())
    }
    ...
    fn novita_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_NOVITA_BASE_URL);
        Ok(())
    }
    ...
    fn fireworks_provider_uses_canonical_defaults() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_FIREWORKS_BASE_URL);
        Ok(())
    }
    ...
    fn sglang_provider_works_without_api_key() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_SGLANG_BASE_URL);
        assert_eq!(config.yunpat_api_key()?, "");
    ...
    fn ollama_provider_uses_local_defaults_without_api_key() -> Result<()> {
        ...
        assert_eq!(config.yunpat_base_url(), DEFAULT_OLLAMA_BASE_URL);
        assert_eq!(config.yunpat_api_key()?, "");
...
"#,
        ...
        assert_eq!(config.yunpat_base_url(), "http://127.0.0.1:11434/v1");
        Ok(())
    }
    ...
    fn ollama_env_overrides_base_url_and_model() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Ollama);
        assert_eq!(config.yunpat_base_url(), "http://ollama.example/v1");
    ...
    fn openrouter_env_api_key_resolves_via_yunpat_api_key() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.yunpat_api_key()?, "or-env-key");
        Ok(())
    }
    ...
    fn novita_env_api_key_resolves_via_yunpat_api_key() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Novita);
        assert_eq!(config.yunpat_api_key()?, "novita-env-key");
        Ok(())
    }
    ...
    fn openrouter_base_url_env_overrides_default() -> Result<()> {
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.yunpat_base_url(), "https://or-mirror.example/v1");
        Ok(())
    }
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Openrouter);
        assert_eq!(config.yunpat_api_key()?, "or-table-key");
        assert_eq!(config.yunpat_base_url(), "https://or-table.example/v1");
        Ok(())
    }
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::Novita);
        assert_eq!(config.yunpat_api_key()?, "novita-table-key");
        assert_eq!(config.yunpat_base_url(), DEFAULT_NOVITA_BASE_URL);
        Ok(())
    }
    ...
    fn has_api_key_for_uses_yunpat_cn_provider_table() -> Result<()> {
        ...
        let mut providers = ProvidersConfig::default();
        providers.yunpat_cn.api_key = Some("cn-file-key".to_string());
        let config = Config {
            providers: Some(providers),
            ..Config::default()
        };
...
"#,
        ...
        let config = Config::load(None, None)?;
        assert_eq!(config.api_provider(), ApiProvider::NvidiaNim);
        assert_eq!(config.yunpat_api_key()?, "nim-table-key");
        assert_eq!(config.yunpat_base_url(), "https://nim-table.example/v1");
        assert_eq!(config.default_model(), DEFAULT_NVIDIA_NIM_MODEL);
        Ok(())
    }
    ...
    #[test]
    fn provider_capability_yunpat_v4_pro_has_1m_window_and_thinking() {
    ...
    #[test]
    fn provider_capability_yunpat_v4_flash_has_1m_window_and_thinking() {
