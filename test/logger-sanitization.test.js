...
describe('Logger - 日志脱敏功能', () => {
    ...
    describe('基本敏感字段脱敏', () => {
        it('应该脱敏password字段', () => {
            ...
            const entry = logger.createLogEntry(LogLevel.INFO, '测试日志', metadata);
        ...
        it('应该脱敏apiKey字段（变体）', () => {
            ...
            const entry1 = logger.createLogEntry(LogLevel.INFO, '测试', metadata1);
            const entry2 = logger.createLogEntry(LogLevel.INFO, '测试', metadata2);
            const entry3 = logger.createLogEntry(LogLevel.INFO, '测试', metadata3);
        ...
        it('应该脱敏secret字段', () => {
            ...
            const entry = logger.createLogEntry(LogLevel.INFO, '测试', metadata);
    ...
    describe('边界情况', () => {
        it('应该处理空对象', () => {
            const entry = logger.createLogEntry(LogLevel.INFO, '测试', {});
            expect(entry.metadata).toEqual({});
        });
        it('应该处理undefined和null', () => {
            const entry1 = logger.createLogEntry(LogLevel.INFO, '测试', undefined);
            const entry2 = logger.createLogEntry(LogLevel.INFO, '测试', null);
            expect(entry1.metadata).toBeUndefined();
            expect(entry2.metadata).toBeNull();
        });
        it('应该处理原始值类型', () => {
            const entry = logger.createLogEntry(LogLevel.INFO, '测试', 'just a string');
