...
export class SimilarityCalculator {
   ...
   * 计算词频(TF)向量
   */
  private getTFVector(tokens: string[], vocabulary: Set<string>): Map<string, number> {
   ...
   * 计算余弦值
   */
  private cosine(vec1: Map<string, number>, vec2: Map<string, number>): number {
    let dotProduct = 0
    let norm1 = 0
    let norm2 = 0
   ...
   * 生成缓存键
   */
  private getCacheKey(text1: string, text2: string, algorithm: string): string {
    // 使用哈希生成键（简化版）
