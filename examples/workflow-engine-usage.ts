 ...
 * 简单的输入验证Agent
 */
class InputValidatorAgent extends Agent<{ technicalDisclosure: string }, { valid: boolean; errors: string[] }> {
  protected async plan(
    input: { technicalDisclosure: string },
    context: any
  ): Promise<any> {
  ...
  async execute(input: { technicalDisclosure: string }): Promise<{ valid: boolean; errors: string[] }> {
...
async function main() {
  ...
  const agents = new Map<string, Agent>([
    ['input-validator', new InputValidatorAgent({
    ...
    ['search-strategy-builder', new SearchStrategyBuilderAgent({
