...
export class FileSystemCheckpointStore {
    ...
    async save(checkpoint) {
        ...
        try {
            // 确保执行目录存在（带超时）
            await withTimeout(fs.mkdir(executionDir, { recursive: true }), TIMEOUT_CONFIG.DIR_TIMEOUT, '创建目录');
            // 序列化为JSON
            const data = JSON.stringify(checkpoint, null, 2);
            // 写入文件（带超时）
            await withTimeout(fs.writeFile(filepath, data, 'utf-8'), TIMEOUT_CONFIG.WRITE_TIMEOUT, '写入文件');
    ...
    async load(checkpointId, executionId) {
        ...
        try {
            ...
            const files = await withTimeout(fs.readdir(executionDir), TIMEOUT_CONFIG.DIR_TIMEOUT, '列出目录');
            const checkpointFiles = files.filter((f) => f.endsWith('.json'));
            // 查找匹配的检查点文件
            for (const file of checkpointFiles) {
                const filepath = path.join(executionDir, file);
                const data = await withTimeout(fs.readFile(filepath, 'utf-8'), TIMEOUT_CONFIG.READ_TIMEOUT, '读取文件');
    ...
    async listResumableExecutions() {
        try {
            const executionDirs = await withTimeout(fs.readdir(this.rootDir), TIMEOUT_CONFIG.DIR_TIMEOUT, '列出根目录');
            const executions = [];
            for (const execDir of executionDirs) {
                const executionDir = path.join(this.rootDir, execDir);
                // 检查是否是目录（带超时）
                const stat = await withTimeout(fs.stat(executionDir), TIMEOUT_CONFIG.DIR_TIMEOUT, '获取文件状态');
                if (!stat.isDirectory()) {
                    continue;
                }
                // 读取该执行的最新检查点
    ...
    async getStats() {
        try {
            ...
            for (const execDir of executionDirs) {
                ...
                const stat = await withTimeout(fs.stat(executionDir), TIMEOUT_CONFIG.DIR_TIMEOUT, '获取文件状态');
                if (stat.isDirectory()) {
                    const files = await withTimeout(fs.readdir(executionDir), TIMEOUT_CONFIG.DIR_TIMEOUT, '列出执行目录');
                    const checkpointFiles = files.filter((f) => f.endsWith('.json'));
                    totalCheckpoints += checkpointFiles.length;
                    for (const file of checkpointFiles) {
                        const filepath = path.join(executionDir, file);
                        const fileStat = await withTimeout(fs.stat(filepath), TIMEOUT_CONFIG.DIR_TIMEOUT, '获取文件状态');
