...
export class ConstitutionalAI {
    ...
    corrector;
    llm;
    constructor(principles, llm = null, config = {}) {
    ...
    async correct(content, violations) {
        ...
        // 验证纠正结果
        const isValid = await this.corrector.verifyCorrection(content, result.correctedContent, violationsToCorrect);
     ...
     * 解决原则冲突
     */
    async resolveConflicts(violations) {
        // 按原则分组违规
     ...
     * 检测冲突的违规
     */
    detectConflicts(violations) {
        const conflicts = [];
        // 按位置分组违规
        const violationsByLocation = new Map();
        for (const violation of violations) {
     ...
     * 获取原则优先级
     */
    getPrinciplePriority(principleId) {
    ...
    generateReportText(report) {
        ...
        if (report.statistics.length > 0) {
            ...
            for (const stat of report.statistics) {
                lines.push(`  ${stat.principleName}: ${stat.violationCount} 违规, ${stat.warningCount} 警告`);
            }
            lines.push('');
        }
        lines.push('='.repeat(80));
