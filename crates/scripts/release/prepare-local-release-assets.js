...
const {
  ...
  CHECKSUM_MANIFEST,
  detectBinaryNames,
} = require("../../npm/yunpat-tui/scripts/artifacts");
...
async function main() {
  ...
  const assets = [
    {
      source: path.join(buildDir, isWindows ? "yunpat.exe" : "yunpat"),
      target: yunpat,
    },
    {
      source: path.join(buildDir, isWindows ? "yunpat-tui.exe" : "yunpat-tui"),
      target: tui,
    },
  ];
  ...
  if (prepareAllAssets) {
    for (const assetName of allAssetNames()) {
      ...
      assets.push({
        source: assetName.startsWith("yunpat-tui")
          ? path.join(buildDir, isWindows ? "yunpat-tui.exe" : "yunpat-tui")
          : path.join(buildDir, isWindows ? "yunpat.exe" : "yunpat"),
        target: assetName,
      });
    }
  }
