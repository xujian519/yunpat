...
> {
  ...
  async execute(
    ...
    try {
      ...
      // 附加被引次数信息（限制并发数为 3，避免触发 API 限流）
      const patentsWithCitations: Array<PatentRecord & { citations: number }> = []
      const batchSize = 3
      for (let i = 0; i < result.patents.length; i += batchSize) {
        const batch = result.patents.slice(i, i + batchSize)
        const batchResults = await Promise.all(
          batch.map(async (patent) => {
            let citations = 0
            try {
              const pubNumber = patent.publicationNumber || patent.applicationNumber || ''
              if (pubNumber) {
                citations = await this.fetchCitationCount(pubNumber)
              }
            } catch {
              // 获取失败时不影响主流程
            }
        ...
        patentsWithCitations.push(...batchResults)
      }
  ...
  /**
   * 通过 Google Patents 搜索获取专利的被引次数
   * 使用 "cite:" 搜索操作符
   */
  private async fetchCitationCount(patentNumber: string): Promise<number> {
    try {
      const { GooglePatentsFetchTool } = await import('./GooglePatentsTool.js')
      const fetchTool = new GooglePatentsFetchTool()
      const result = await fetchTool.execute({ query: `cite:${patentNumber}`, page: 1 }, {} as any)
