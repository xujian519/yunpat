...
impl ViewStack {
    ...
    pub fn push<V: ModalView + 'static>(&mut self, view: V) {
        ...
        tracing::debug!(target: "yunpat_tui::view_stack", action = "push", kind = ?kind, depth = self.views.len(), "view pushed");
    }
    ...
    pub fn push_boxed(&mut self, view: Box<dyn ModalView>) {
        ...
        tracing::debug!(target: "yunpat_tui::view_stack", action = "push_boxed", kind = ?kind, depth = self.views.len(), "view pushed");
    }

    pub fn pop(&mut self) -> Option<Box<dyn ModalView>> {
        let popped = self.views.pop();
        if let Some(view) = popped.as_ref() {
            tracing::debug!(target: "yunpat_tui::view_stack", action = "pop", kind = ?view.kind(), depth = self.views.len(), "view popped");
        }
        popped
    }
        ...
        match action {
            ...
            ViewAction::Close => {
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "yunpat_tui::view_stack", action = "close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
            ...
            ViewAction::EmitAndClose(event) => {
                events.push(event);
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "yunpat_tui::view_stack", action = "emit_and_close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
        }
        events
