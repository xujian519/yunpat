...
use yunpat_config::{CliRuntimeOverrides, ConfigStore};
use yunpat_core::Runtime;
use yunpat_execpolicy::ExecPolicyEngine;
use yunpat_hooks::{HookDispatcher, JsonlHookSink, StdoutHookSink};
use yunpat_mcp::McpManager;
use yunpat_protocol::{
...
struct AppState {
    ...
    config: Arc<RwLock<yunpat_config::ConfigToml>>,
...
async fn tool_handler(
    ...
    match runtime
        .invoke_tool(req.call, yunpat_execpolicy::AskForApproval::OnRequest, &cwd)
        .await
    {
        Ok(value) => Json(value),
    ...
    match req {
        ...
        AppRequest::ThreadLoadedList => {
            ...
            let response = runtime
                .handle_thread(yunpat_protocol::ThreadRequest::List(
                    yunpat_protocol::ThreadListParams {
...
async fn persist_config(state: &AppState, config: yunpat_config::ConfigToml) -> Result<()> {
