...
> {
  readonly metadata = {
    ...
    description:
      '解析专利官文（审查意见通知书、驳回决定等），提取结构化字段（不依赖外部OCR服务）',
  ...
  async execute(
    ...
    if (input.filePath.toLowerCase().endsWith('.pdf')) {
      ...
      input.filePath.match(/\.(png|jpg|jpeg|bmp|tiff)$/i)
    ) {
      // 使用 OCR 工具
      if (input.useOcr) {
    ...
    const detectedDocType =
      input.docType || this.detectDocType(text, input.filePath);
   ...
   * 提取审查意见通知书字段
   */
  private extractReviewOpinionFields(
    text: string,
    fields: OfficialDocFields
  ): void {
    // 提取审查意见摘要
    ...
    const deadlineMatch = text.match(
      /答复期限[：:]\s*([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日)/i
    );
  ...
  private extractRejectionFields(text: string, fields: OfficialDocFields): void {
    ...
    const dateMatch = text.match(
      /决定日[期][：:]\s*([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日)/i
    );
    if (dateMatch && dateMatch[1]) {
      fields.decisionDate = dateMatch[1]
        .replace('年', '-')
        .replace('月', '-')
        .replace('日', '');
    }
  ...
  private extractPaymentFields(text: string, fields: OfficialDocFields): void {
    ...
    // 提取缴费截止日期
    const deadlineMatch = text.match(
      /截止日期[：:]\s*([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日)/i
    );
    if (deadlineMatch && deadlineMatch[1]) {
   ...
   * 提取复审无效决定字段
   */
  private extractReexaminationFields(
    text: string,
    fields: OfficialDocFields
  ): void {
    // 提取决定类型
    if (text.includes('复审')) {
      fields.decisionDate = text.includes('维持')
        ? '维持驳回决定'
        : '撤销驳回决定';
    } else if (text.includes('无效')) {
      fields.decisionDate = text.includes('宣告无效')
        ? '宣告专利权无效'
        : '维持专利权有效';
    }
