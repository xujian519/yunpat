...
describe('Gateway - 额外分支覆盖测试', () => {
  ...
  describe('sendOutput - 覆盖未测试分支', () => {
    ...
    it('应该处理未启用审计日志的情况', async () => {
      ...
      await expect(
        noAuditGateway.sendOutput(output, OutputTargetType.TERMINAL)
    ...
    it('应该处理没有 auditStore 的情况', async () => {
      ...
      await expect(
        noStoreGateway.sendOutput(output, OutputTargetType.TERMINAL)
      ).resolves.toBeUndefined()
    })
  ...
  describe('OAuth 辅助方法分支', () => {
    ...
    it('handleOAuthCallback 应该在管理器未配置时抛出错误', async () => {
      ...
      await expect(
        noOAuthGateway.handleOAuthCallback(
          'github',
          'code',
          'state',
          'http://localhost:3000/callback'
    ...
    it('handleOAuthCallback 应该调用 OAuth 管理器', async () => {
      ...
      const result = await gateway.handleOAuthCallback(
        'github',
        'code',
        'state',
        'http://localhost:3000/callback'
  ...
  describe('writeAuditLog - 分支覆盖', () => {
    it('应该在没有 auditStore 时输出到控制台', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith('[审计日志]', expect.stringContaining('test'))
  ...
  describe('authorize - 边界情况', () => {
    it('应该处理空权限列表', async () => {
      ...
      const result = await testGateway.authorize({ type: 'read', resource: 'file' }, [])
    ...
    it('应该处理通配符资源权限', async () => {
      const result = await gateway.authorize({ type: 'write', resource: 'any-resource' }, [
    ...
    it('应该处理通配符操作权限', async () => {
      const result = await gateway.authorize({ type: 'any-action', resource: 'file' }, [
