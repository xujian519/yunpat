...
export class DynamicReplanner {
     ...
     * 检测是否需要重规划
     */
    async shouldReplan(plannedState, actualState) {
        // 1. 检测失败（最高优先级）
        ...
        if (this.config.enableQualityDropDetection) {
            const qualityDrop = plannedState.qualityMetrics.overallQuality - actualState.qualityMetrics.overallQuality;
            if (qualityDrop > this.config.qualityDropThreshold) {
                return {
                    ...
                    trigger: {
                        ...
                        description: '检测到质量下降',
                        condition: (state) => plannedState.qualityMetrics.overallQuality - state.qualityMetrics.overallQuality >
            ...
            if (timeOverrun > 1 + this.config.timeoutTolerance) {
                return {
                    ...
                    trigger: {
                        ...
                        description: '检测到超时',
                        condition: (state) => state.elapsedTime / state.resourceUsage.estimatedTime >
        ...
        if (this.config.enableDeviationDetection) {
            const deviationReport = await this.deviationDetector.detectDeviations(plannedState, actualState, {
            ...
            if (deviationReport.hasDeviation) {
                return {
                    ...
                    trigger: {
                        ...
                        description: '检测到偏离',
                        condition: (_state) => deviationReport.overallDeviationScore > this.config.deviationThreshold,
                    },
    ...
    async replan(currentPlan, executionState, trigger) {
        ...
        // 1. 检测偏离
        const deviationReport = await this.deviationDetector.detectDeviations(this.createPlannedState(currentPlan), executionState);
        // 2. 创建重规划上下文
        const context = {
        ...
        // 3. 选择恢复策略
        const strategy = await this.strategySelector.selectBestStrategy(deviationReport, context, this.config.preferredStrategies);
     ...
     * 计算置信度
     */
    calculateConfidence(adjustment, deviationReport) {
        let confidence = 0.5; // 基础置信度
        // 根据偏离严重程度调整
        if (deviationReport.hasDeviation) {
            const maxSeverity = deviationReport.deviations.reduce((max, d) => (d.severity === 'severe' ? d : max), {});
     ...
     * 生成推理说明
     */
    generateReasoning(deviationReport, strategy, adjustment) {
        const parts = [];
        // 偏离描述
        if (deviationReport.hasDeviation) {
