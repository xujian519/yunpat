...
export class CardRetriever {
    ...
    async search(query, options = {}) {
        const { mode = 'hybrid', limit = 5, minQuality = 0, domain, concept, tags, minSimilarity = 0.3, } = options;
...
${prompt}`;
    ...
    updateIndex(card) {
        if (!this.conceptIndex.has(card.concept))
            this.conceptIndex.set(card.concept, new Set());
        this.conceptIndex.get(card.concept).add(card.id);
        if (!this.domainIndex.has(card.domain))
            this.domainIndex.set(card.domain, new Set());
