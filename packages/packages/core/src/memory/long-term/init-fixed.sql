-- PostgreSQL + pgvector 初始化脚本（修复版）
--
-- 使用方法：
--   psql -U yunpat -d yunpat -f init-fixed.sql
...
CREATE TABLE IF NOT EXISTS memories (
  ...
  embedding vector(1024) NOT NULL, -- 修复：使用 vector 类型而不是 text
  metadata JSONB,
...
VALUES
  ('patent', '专利撰写的关键在于权利要求书的撰写', '[0.1, 0.2, 0.3]'::vector(1024), '{"agent": "writer", "tags": ["专利", "撰写"]}'),
  ('patent', '专利检索是专利申请前的重要步骤', '[0.2, 0.3, 0.4]'::vector(1024), '{"agent": "researcher", "tags": ["专利", "检索"]}'),
  ('conversation', '今天天气不错', '[0.9, 0.8, 0.7]'::vector(1024), '{"tags": ["闲聊"]}')
...
SELECT
  ...
  '已创建表: memories (使用 vector 类型), graph_entities, graph_relations, user_profiles' as tables,
