...
export class ToolUsageTracker extends EventEmitter {
    ...
    storagePath;
    config = {};
    constructor(storagePathOrConfig) {
        super();
        // 支持两种调用方式：
     ...
     * 获取工具推荐
     */
    getRecommendations(userInput, availableTools) {
        ...
        for (const toolName of availableTools) {
            ...
            // 基于工具名称匹配
            if (inputLower.includes(toolName.toLowerCase()) ||
     ...
     * 获取使用趋势
     */
    getUsageTrends(toolName, days = 7) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        const recentRecords = this.records.filter((r) => r.toolName === toolName && r.timestamp >= cutoffDate);
    ...
    analyzeSelectionAccuracy() {
        ...
        if (failed.length > 0) {
            ...
            if (failureRate > 0.2) {
                improvements.push(`失败率较高(${(failureRate * 100).toFixed(1)}%)，建议加强参数验证和错误处理`);
            }
        }
        // 分析执行时间
        const slowOperations = this.records.filter((r) => r.result.executionTime > 5000);
        if (slowOperations.length > 0) {
            improvements.push(`${slowOperations.length}个操作执行时间超过5秒，建议优化性能或使用异步处理`);
        }
        // 计算准确性（成功率）
    ...
    extractUseCase(userInput) {
        const input = userInput.toLowerCase();
        if (input.includes('pdf') && input.includes('markdown'))
            return 'PDF转Markdown';
        if (input.includes('pdf') && input.includes('文本'))
            return 'PDF文本提取';
        if (input.includes('excel') && input.includes('json'))
            return 'Excel转JSON';
        if (input.includes('网页') && input.includes('截图'))
            return '网页截图';
        if (input.includes('图片') && input.includes('文字'))
            return '图片OCR';
        if (input.includes('音频') && input.includes('文字'))
            return '音频转写';
        return '通用处理';
    }
    isUseCaseMatch(useCase, userInput) {
    ...
    generateRecommendationReason(toolName, stats) {
        const reasons = [];
    ...
    saveRecords() {
        try {
            ...
            fs.writeFileSync(this.storagePath, JSON.stringify(this.records.slice(-1000), null, 2), // 只保存最近1000条
            'utf-8');
        }
        catch (error) {
