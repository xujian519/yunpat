...
use crate::config::{COMMON_YUNPAT_MODELS, normalize_model_name};
...
pub fn yunpat_links(app: &mut App) -> CommandResult {
    let locale = app.ui_locale;
...
mod tests {
    ...
    #[test]
    fn test_model_change_accepts_future_yunpat_model() {
        let mut app = create_test_app();
    ...
    #[test]
    fn test_yunpat_links() {
        let mut app = create_test_app();
        let result = yunpat_links(&mut app);
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
