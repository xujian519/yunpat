...
#   2. `npm/yunpat-tui/package.json` `version` matches the workspace
#      `version` in the root `Cargo.toml`.
#   3. Internal `yunpat-*` path dependency pins match the workspace version.
...
internal_dep_drift="$(
  grep -nE 'yunpat-[a-z-]+[[:space:]]*=[[:space:]]*\{[^}]*version[[:space:]]*=[[:space:]]*"' crates/*/Cargo.toml \
    | grep -v "version[[:space:]]*=[[:space:]]*\"${workspace_version}\"" || true
)"
if [[ -n "${internal_dep_drift}" ]]; then
  echo "::error::Internal yunpat-* path dependency versions must match workspace version ${workspace_version}:" >&2
...
if ! cargo metadata --locked --format-version 1 --no-deps >/dev/null 2>&1; then
  echo "::error::Cargo.lock is out of sync with the manifests. Run 'cargo update -p yunpat-tui' or 'cargo build' and commit the result." >&2
  fail=1
fi
