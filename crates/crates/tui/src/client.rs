...
use crate::llm_client::{LlmClient, LlmError, extract_retry_after, with_retry};
use crate::logging;
...
impl DeepSeekClient {
    ...
    pub fn new(config: &Config) -> Result<Self> {
        let api_key = config.yunpat_api_key()?;
        let base_url = config.yunpat_base_url();
...
impl LlmClient for DeepSeekClient {
    ...
    fn create_message(
        ...
        let this = self.clone();
        Box::pin(async move { this.create_message_chat(&request).await })
    }
    ...
    fn create_message_stream(
        ...
        let this = self.clone();
        Box::pin(async move { this.handle_chat_completion_stream(request).await })
    }
...
mod tests {
    ...
    #[test]
    fn api_url_routes_beta_paths_from_any_yunpat_base() {
        assert_eq!(
    ...
    #[test]
    fn reasoning_effort_uses_yunpat_top_level_thinking_parameter() {
        let mut body = json!({});
    ...
    #[test]
    fn parse_usage_reads_yunpat_cache_and_reasoning_tokens() {
