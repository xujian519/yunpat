...
export class DeviationDetector {
  ...
  async detectDeviations(
    ...
    // 2. 检测质量偏离
    const qualityDeviations = this.detectQualityDeviations(
  ...
  private detectResourceDeviations(
    plannedResources: ResourceUsage,
    ...
    // 检测时间偏离
    const timeDeviation = this.calculateResourceDeviation(
      plannedResources.timeElapsed,
  ...
  private detectDependencyDeviation(
    ...
    return {
      type: 'dependency_deviation' as DeviationType,
