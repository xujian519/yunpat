...
export class BatchProcessorOptimizer {
    ...
    partitionIntoBatches(texts, model, maxBatchSize) {
        ...
        // 计算统计信息
        const batchTokenCounts = batches.map((batch) => this.tokenCounter.calculateTotalTokens(batch, model));
        return {
    ...
    adjustBatchSize(previousBatches, model) {
        ...
        // 分析历史批次的 Token 使用情况
        const tokenUsages = previousBatches.map((batch) => this.tokenCounter.calculateTotalTokens(batch, model));
    ...
    getBatchStatistics(batches, model) {
        ...
        const variance = tokenCounts.reduce((sum, count) => sum + Math.pow(count - avgTokensPerBatch, 2), 0) /
            tokenCounts.length;
        return {
            totalBatches: batches.length,
