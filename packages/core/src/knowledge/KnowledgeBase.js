...
export class KnowledgeBase {
    ...
    async search(query, options = {}) {
        const opts = {
            ...
            semanticWeight: options.semanticWeight ?? this.config.defaultSearchOptions.semanticWeight ?? 0.5,
        };
        // 获取候选条目（通过索引过滤）
    ...
    getCandidates(options) {
        ...
        if (options.categories && options.categories.length > 0) {
            const categoryIds = options.categories.flatMap((cat) => Array.from(this.categoryIndex.get(cat) ?? []));
