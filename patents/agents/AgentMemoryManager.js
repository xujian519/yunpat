...
import { loadMemoryConfig, validateMemoryConfig, } from '../../packages/core/src/memory/config.js';
/**
...
export class AgentMemoryManager {
     ...
     * 为 Agent 注册 Token 窗口
     */
    registerTokenWindow(agentName, config) {
     ...
     * 语义搜索（跨所有 Agent）
     */
    async searchMemories(query, topK = 5, types) {
        if (!this.isInitialized) {
 ...
 * 快速初始化全局记忆层
 */
export async function initGlobalMemory(config) {
