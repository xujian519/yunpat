    ...
    async plan(input, context) {
        ...
        try {
            ...
            if (classifyText) {
                ...
                console.log(`[PatentAnalyzerAgent] IPC 分类: ${ipcClassification.classifications.map((c) => `${c.section}(${c.description})`).join(', ')}`);
            }
        }
        catch (e) {
            console.warn('[PatentAnalyzerAgent] patent-core IPC 分类失败:', e.message);
        }
        const ipcContext = ipcClassification
            ? `\n\n## patent-core IPC 分类\n${ipcClassification.classifications.map((c) => `- ${c.section}部: ${c.description}`).join('\n')}`
            : '';
        // 使用 LLM 制定分析策略
...
${ipcContext}`,
    ...
    async reflect(output, context) {
        ...
        const ipcInfo = ipcVerification
            ? `\n\n## IPC 分类验证\n${ipcVerification.classifications.map((c) => `- ${c.section}: ${c.description}`).join('\n')}`
            : '';
...
建议数量：${output.recommendations.length}${ipcInfo}`,
     ...
     * - 分析引用数据、法律状态、市场数据
     */
    async analyzeValue(plan, context) {
...
分析策略：${plan?.strategy || '未指定'}`,
            ...
            return {
                valueAssessment: {
                    highValuePatents: input.targetPatents?.slice(0, 3).map((p) => ({
                        patentNumber: p,
                        score: 75,
     ...
     * - 识别新兴技术和衰退技术
     */
    async analyzeTrend(plan, context) {
...
分析策略：${plan?.strategy || '未指定'}`,
     ...
     * - 分析专利布局和技术优势
     */
    async analyzeCompetitor(plan, context) {
        console.log(`   🏢 执行竞品分析...`);
        const input = context.input;
        try {
            const response = await context.llm.chat({
...
分析策略：${plan?.strategy || '未指定'}`,
            ...
            return {
                competitorAnalysis: {
                    rankings: input.competitors?.map((c) => ({
                        company: c,
                        patentCount: 50,
                        marketShare: 20,
                        strength: ['在该领域有专利布局'],
     ...
     * - 生成可视化专利地图
     */
    async analyzeLandscape(plan, context) {
