...
async function main() {
  ...
  console.log(`   主要分类: ${electronics.classifications[0].section} - ${electronics.classifications[0].description}`);
  ...
  console.log(`   主要分类: ${mechanical.classifications[0].section} - ${mechanical.classifications[0].description}`);
  console.log(`   是否使用 Fallback: ${mechanical.fallback ? '是' : '否'}\n`);

  // 示例 3: 化学技术发明
  console.log('3️⃣ 化学技术发明:');
  const chemical = await classifyIpc(
    '聚合物催化剂涂料组合物的制备方法，包括纳米填料分散工艺，' +
    '用于提高涂层的耐候性和附着力'
  );
  console.log(`   主要分类: ${chemical.classifications[0].section} - ${chemical.classifications[0].description}`);
  ...
  console.log(`   主要分类: ${computer.classifications[0].section} - ${computer.classifications[0].description}`);
  console.log(`   是否使用 Fallback: ${computer.fallback ? '是' : '否'}\n`);

  // 示例 5: 多领域交叉发明
  console.log('5️⃣ 多领域交叉发明:');
  const multiDomain = await classifyIpc(
    '智能传感器网络通信与机械传动系统的集成控制方法，' +
    '结合了电子信号处理和机械结构设计'
  );
  console.log(`   分类结果:`);
