 ...
 * 步骤格式类型
 */
export var StepFormat;
(function (StepFormat) {
    /** 编号格式：步骤1、步骤2 */
    StepFormat["NUMBERED"] = "numbered";
    /** 列表格式：1. 2. 3. */
    StepFormat["LIST"] = "list";
    /** 项目符号：• - * */
    StepFormat["BULLET"] = "bullet";
    /** 自动检测 */
    StepFormat["AUTO"] = "auto";
...
export class ChainOfThoughtStrategy {
     ...
     * @returns 推理结果
     */
    async reason(problem, context, format) {
     ...
     * 生成 CoT 推理过程
     */
    async generateCoT(problem, context, format) {
     ...
     * 构建 CoT 提示词
     */
    buildCoTPrompt(problem, context, format) {
     ...
     * 获取步骤格式描述
     */
    getStepFormatDescription(format) {
        switch (format) {
...
结论：[最终答案]`;
     ...
     * 解析推理步骤
     */
    parseReasoningSteps(content, _format) {
        const steps = [];
        // 定义多种步骤匹配模式
        const patterns = [
            // "步骤1"、"步骤2"格式
        ...
        for (const pattern of patterns) {
            ...
            if (matches.length > 0) {
                for (let i = 0; i < matches.length; i++) {
                    ...
                    const description = lines[0]?.replace(/^[步骤\d\s\.\):： \-•*]+/, '').trim() || stepContent.substring(0, 50);
                    // 提取推理内容（去除标记后的内容）
    ...
    extractConclusion(content) {
        ...
        if (paragraphs.length > 0) {
            ...
            const hasConclusionKeyword = conclusionKeywords.some((keyword) => lastParagraph.includes(keyword));
     ...
     * 计算置信度
     */
    calculateConfidence(steps, conclusion) {
        let confidence = 0.0;
        // 因素1：步骤数量（3-7步为最佳）
     ...
     * 生成摘要
     */
    generateSummary(steps, conclusion) {
     ...
     * @returns 推理步骤生成器
     */
    async *reasonStream(problem, context) {
     ...
     * @returns 推理结果数组
     */
    async reasonBatch(problems, context) {
        const results = [];
 ...
 * @returns CoT 策略实例
 */
export function createChainOfThought(llm, config) {
