...
export function generateCardId(question, concept) {
...
export function markdownToCard(content, id) {
    ...
    for (const line of lines) {
        if (line.startsWith('- 来源问题:'))
            question = line.replace(/^- 来源问题:\s*/, '');
        else if (line.startsWith('- 质量分:'))
            quality = parseFloat(line.replace(/^- 质量分:\s*/, '')) || 0.5;
        else if (line.startsWith('- 生成时间:'))
            timestamp = line.replace(/^- 生成时间:\s*/, '');
        else if (line.startsWith('- 概念:'))
            concept = line.replace(/^- 概念:\s*/, '');
        else if (line.startsWith('- 领域:'))
            domain = line.replace(/^- 领域:\s*/, '');
