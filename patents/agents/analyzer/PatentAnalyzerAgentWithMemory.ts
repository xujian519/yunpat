 ...
 * PatentAnalyzerAgent with Memory Layer
 */
export class PatentAnalyzerAgentWithMemory extends Agent<PatentAnalysisInput, PatentAnalysisOutput> {
  ...
  constructor(config: {
    ...
    super({
      ...
      description:
        config.description || '专利分析智能体 - 集成记忆层（RAG + Token窗口）',
    });
...
${analysisResult.recommendations.map(r => `- ${r}`).join('\n')}
   ...
   * 语义搜索历史分析结果
   */
  async searchAnalyses(query: string, topK: number = 5): Promise<
    Array<{
      content: string;
      similarity: number;
      metadata: any;
