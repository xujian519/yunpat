...
mockChatWithSchema.mockImplementation(async (messages: any, _schema: any) => {
  // 添加延迟以确保时间戳变化（至少1ms）
  ...
  // 检查是否是ResultAggregator调用
  const isAggregationCall = messages.some(
    (m: any) => m.content?.includes('结果整合器') || m.content?.includes('聚合')
  )

  if (isAggregationCall) {
    return {
      reply: '# 任务完成\n\n已成功完成所有步骤。',
      artifacts: [],
...
describe('OrchestratorAgent端到端集成测试', () => {
  ...
  beforeEach(() => {
    ...
    // 注册 Mock Agent（动态 import 在测试环境可能加载真实 agent 但缺少完整依赖）
