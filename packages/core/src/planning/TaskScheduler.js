...
export class TaskScheduler {
     ...
     * 将任务分组为可并行的组
     */
    groupParallelTasks(executionOrder, dependencies) {
        const groups = [];
     ...
     * 最大并行分组（考虑资源约束）
     */
    maxParallelGrouping(executionOrder, dependencies) {
        const groups = [];
        const processed = new Set();
        ...
        for (const taskId of executionOrder) {
            ...
            for (const candidateId of executionOrder) {
                ...
                // 检查依赖关系
                const hasDependency = group.some((id) => this.checkDependency(id, candidateId, dependencies));
                if (hasDependency) {
                    continue;
                }
                // 检查资源约束
     ...
     * 检查两个任务是否有依赖关系
     */
    checkDependency(taskIdA, taskIdB, dependencies) {
        // 检查 A -> B
     ...
     * 获取任务需要的资源
     */
    getRequiredResources(taskId, dependencies) {
     ...
     * 检查资源冲突
     */
    checkResourceConflict(usedResources, requiredResources) {
     ...
     * 调整顺序以尊重依赖关系
     */
    respectDependencies(order, dependencies) {
        const adjusted = [];
     ...
     * 计算并行完成时间
     */
    calculateParallelCompletionTime(parallelGroups, plan) {
        let totalTime = 0;
        for (const group of parallelGroups) {
            // 每组的时间 = 组中最长的任务时间
            const maxGroupTime = Math.max(...group.map((id) => {
     ...
     * 计算资源利用率
     */
    calculateResourceUtilization(parallelGroups) {
        if (parallelGroups.length === 0) {
            return 0;
        }
        const avgTasksPerGroup = parallelGroups.reduce((sum, group) => sum + group.length, 0) / parallelGroups.length;
     ...
     * 获取下一个可执行的任务
     */
    getNextExecutableTasks(plan, completedTasks) {
        const nextTasks = [];
     ...
     */
    isPlanComplete(plan) {
        return plan.subGoals.every((goal) => goal.tasks.every((task) => task.status === TaskStatus.COMPLETED));
    }
    /**
     * 获取执行进度
     */
