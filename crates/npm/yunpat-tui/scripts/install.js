...
function binaryPaths() {
  const { yunpat, tui } = detectBinaryNames();
  const releaseDir = releaseBinaryDirectory();
  return {
    yunpat: {
      asset: yunpat,
      target: path.join(releaseDir, process.platform === "win32" ? "yunpat.exe" : "yunpat"),
    },
    tui: {
      asset: tui,
      target: path.join(releaseDir, process.platform === "win32" ? "yunpat-tui.exe" : "yunpat-tui"),
    },
  };
}
...
function logInfo(message) {
  ...
  process.stderr.write(`yunpat-tui: ${message}\n`);
}
...
function createProgressReporter(assetName, totalBytes) {
  ...
  const render = (final) => {
    if (totalBytes && totalBytes > 0) {
      ...
      const line = `yunpat-tui: downloading ${assetName}: ${formatMb(received)} / ${formatMb(totalBytes)} MB (${pct}%)`;
      if (interactive) {
      ...
      const line = `yunpat-tui: downloading ${assetName}: ${formatMb(received)} MB downloaded`;
      if (interactive) {
        process.stderr.write(`${line}\r`);
      } else {
        process.stderr.write(`${line}\n`);
  ...
  return {
    ...
    finish() {
      ...
      process.stderr.write(`yunpat-tui: ${assetName} ... done.\n`);
    },
  };
}
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.once("connect", () => {
      const lines = [
        ...
        "User-Agent: yunpat-tui-installer",
...
async function run() {
  ...
  await Promise.all([
    ensureBinary(paths.yunpat.target, paths.yunpat.asset, version, repo, getChecksums),
...
if (require.main === module) {
  run().catch((error) => {
    console.error("yunpat-tui install failed:", error.message);
