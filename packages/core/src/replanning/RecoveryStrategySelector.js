...
export class RecoveryStrategySelector {
                    ...
                    modifications: [
                        {
                            ...
                            changes: {
                                newEstimate: {
                                    duration: (context.originalPlan.subGoals.find((g) => g.id === currentGoal)
                                        ?.estimatedDuration ?? 0) * 0.6,
                                    tokens: (context.originalPlan.subGoals.find((g) => g.id === currentGoal)
    ...
    async selectBestStrategy(deviationReport, context, preferredStrategies) {
        ...
        // 评分并选择最佳策略
        const scoredStrategies = await this.scoreStrategies(candidateStrategies, deviationReport, context);
        // 按分数降序排序
    ...
    getMaxSeverity(deviationReport) {
        ...
        const maxDeviation = deviationReport.deviations.reduce((max, d) => severityOrder[d.severity] > severityOrder[max.severity] ? d : max);
