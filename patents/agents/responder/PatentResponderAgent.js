...
export class PatentResponderAgent extends Agent {
    ...
    async plan(input, context) {
        ...
        try {
            ...
            console.log(`[PatentResponderAgent] 推荐策略: ${recommendedStrategies.strategies.map((s) => `${s.strategy_type}(${s.confidence})`).join(', ')}`);
        }
        catch (e) {
            console.warn('[PatentResponderAgent] patent-core OA 解析失败，回退到纯 LLM 模式:', e.message);
        }
        // 构建增强的提示词
        const oaContext = parsedOa
            ? `\n\n## patent-core OA 分析\n类型: ${parsedOa.oa_type}\n受影响权利要求: ${parsedOa.affected_claims.join(', ')}\n引用文献: ${parsedOa.citations.map((c) => c.document_number).join(', ')}\n审查员论点: ${parsedOa.examiner_arguments.substring(0, 500)}`
            : '';
        const strategyContext = recommendedStrategies
            ? `\n\n## 推荐答复策略（patent-core）\n${recommendedStrategies.strategies.map((s) => `- ${s.strategy_type} (置信度: ${s.confidence}): ${s.reasoning}`).join('\n')}`
            : '';
    ...
    async reflect(output, context) {
        ...
        try {
            const claimDrafts = output.response.amendedClaims.map((c, i) => ({
                id: String(i + 1),
                claim_type: (i === 0 ? 'Independent' : 'Dependent'),
            ...
            console.warn('[PatentResponderAgent] patent-core 质量评估失败:', e.message);
        }
        const coreInfo = coreAssessment
            ? `\n\n## patent-core 规则化评估\n综合: ${coreAssessment.overall_score}\n${coreAssessment.issues?.map((i) => `- [${i.severity}] ${i.suggestion}`).join('\n') || ''}`
            : '';
...
授权成功率预测：${output.metrics.allowanceProbability}%${coreInfo}`,
        ...
        return {
            ...
            coreAssessment,
            improvementSuggestions: coreAssessment?.issues?.map((i) => i.suggestion) || [],
        };
    }
    /**
     * 制定答复策略
     */
    async formulateStrategy(plan, context) {
...
请制定最佳答复策略，并以 JSON 格式返回。`,
     ...
     * 生成意见陈述书
     */
    async generateWrittenArgument(plan, context) {
...
`,
     ...
     * 生成修改后的权利要求
     */
    async generateAmendedClaims(plan, context) {
...
`,
     ...
     * 生成修改对照页
     */
    async generateAmendmentComparison(plan, context) {
...
`,
     ...
     * 计算授权成功率
     */
    calculateAllowanceProbability(plan, strategy) {
        // 基于多个因素计算授权成功率
     ...
     * 计算答复质量评分
     */
    calculateQualityScore(plan, writtenArgument) {
        let score = 70; // 基础分
        // 意见陈述书长度评估
     ...
     * 计算审查员接受可能性
     */
    calculateExaminerAcceptance(plan, strategy) {
        // 基于策略和论点质量计算
        let acceptance = 60;
