...
export class ExternalFactChecker {
    ...
    async verifyClaim(claim, options) {
        if (!this.apiKey) {
            throw new FactCheckError('API Key 未配置，无法使用外部验证功能', undefined, 'ExternalFactChecker');
        }
        // 检查缓存
        if (this.enableCache) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new FactCheckError(`API 请求失败: ${response.status} ${response.statusText}`, response.status, 'Google Fact Check API');
            }
            ...
            throw new FactCheckError(`验证请求失败: ${error instanceof Error ? error.message : String(error)}`, undefined, 'ExternalFactChecker');
        }
    }
    /**
     * 批量验证声明
     ...
     * @returns 事实验证结果列表
     */
    async verifyClaims(claims, options) {
        const results = [];
     ...
     * @returns 事实验证结果
     */
    parseResponse(data, claim, source) {
 ...
 * @returns 共识状态
 */
export function calculateConsensus(results) {
    if (results.length === 0) {
        return 'NO_CONSENSUS';
    }
