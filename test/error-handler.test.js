...
import { withErrorHandling, withErrorHandlingOrDefault, withErrorHandlingResult, withRetry, withBatchErrorHandling, withTimeout, } from '../patents/core/error-handler.js';
describe('错误处理器', () => {
    let logger;
    beforeEach(() => {
    ...
    describe('withErrorHandling', () => {
        ...
        it('应该在操作失败时抛出错误', async () => {
            ...
            await expect(withErrorHandling(operation, '测试操作', logger, { throwOnError: true })).rejects.toThrow('测试操作 失败');
        });
        it('应该记录错误日志', async () => {
        ...
        it('应该使用自定义错误消息', async () => {
            ...
            await expect(withErrorHandling(operation, '测试操作', logger, {
    ...
    describe('withRetry', () => {
        ...
        it('应该在达到最大重试次数后失败', async () => {
            ...
            await expect(withRetry(operation, '测试操作', logger, {
                maxRetries: 2,
                delayMs: 10,
            })).rejects.toThrow();
        });
    ...
    describe('组合使用', () => {
        it('应该支持withRetry和withTimeout组合', async () => {
            ...
            const result = await withTimeout(() => withRetry(operation, '测试操作', logger, {
                maxRetries: 3,
                delayMs: 10,
            }), 500, '测试操作', logger);
            expect(result).toBe('success');
