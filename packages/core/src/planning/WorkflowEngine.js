...
export class WorkflowEngine {
    ...
    cleanupOldWorkflows() {
        ...
        // 获取所有执行并按开始时间排序
        const executions = Array.from(this.activeWorkflows.values()).sort((a, b) => a.startTime.getTime() - b.startTime.getTime());
        // 删除最旧的20%
    ...
    async saveCheckpoint(execution) {
        ...
        try {
            await this.config.checkpointManager.saveCheckpoint('workflow-engine', execution.executionId, execution.currentStepIndex, Object.fromEntries(execution.stepOutputs), {
                workflowId: execution.workflowId,
                currentStepIndex: execution.currentStepIndex,
                stepResults: execution.stepResults,
                initialInput: execution.initialInput,
            }, { paused: execution.paused, workflow: execution.workflow }, ['workflow'], `工作流 ${execution.workflowId} 步骤 ${execution.currentStepIndex}`);
        }
        catch (error) {
