...
export class ToolSelectionOptimizer {
     ...
     * 优化工具选择提示
     */
    optimizeToolSelectionPrompt(userInput, availableTools, context) {
        // 1. 生成增强的工具描述
        ...
        // 3. 获取工具推荐（基于使用历史）
        const recommendations = toolUsageTracker.getRecommendations(userInput, availableTools.map((t) => t.metadata.name));
        // 4. 生成优化后的提示
        return this.generateOptimizedPrompt(userInput, availableTools, enhancedMetadata, relevantExamples, recommendations, context);
    }
    /**
     * 生成优化提示
     */
    generateOptimizedPrompt(userInput, availableTools, enhancedMetadata, relevantExamples, recommendations, context) {
        let prompt = `
# 工具选择辅助系统
...
`;
     ...
     * 格式化工具描述
     */
    formatToolDescription(metadata) {
        let desc = `
### ${metadata.name}
...
`;
     ...
     * 记录工具使用
     */
    recordToolUsage(toolName, userInput, toolParameters, result, context) {
        const record = {
            id: '',
            timestamp: new Date(),
            toolName,
     ...
     * 生成工具描述文档
     */
    generateToolDocumentation(availableTools) {
        const enhancedMetadata = this.descriptionEnhancer.enhanceTools(availableTools);
