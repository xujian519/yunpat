    ...
    async before(ctx) {
        ...
        for (const requiredPermission of permissions) {
            if (!userPermissions.includes(requiredPermission)) {
                throw new Error(`Permission denied: '${requiredPermission}' required for tool '${ctx.tool.metadata.name}'`);
            }
        }
    }
    async onError(ctx, error) {
...
export class RateLimitMiddleware {
    ...
    async before(ctx) {
        ...
        if (!limiter.check()) {
            ...
            throw new Error(`Rate limit exceeded for tool '${ctx.tool.metadata.name}'. ` +
