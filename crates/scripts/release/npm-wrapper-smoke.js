...
const packageDir = path.join(repoRoot, "npm", "yunpat-tui");
...
async function main() {
  const tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "yunpat-npm-smoke-"));
  ...
  try {
    ...
    await runCommand("npx", ["--no-install", "yunpat", "doctor", "--help"], {
      cwd: installDir,
      env,
    });
    await runCommand("npx", ["--no-install", "yunpat-tui", "--help"], {
      cwd: installDir,
      env,
    });
