/**
 * 实际智能体集成示例 - 3种集成方式（纯JavaScript版本）
 *
 * 演示如何在真实智能体中使用工具选择优化器
 */

import { toolSelectionOptimizer } from '../packages/core/dist/index.js'

// 模拟LLM
class MockLLM {
  async chat(messages) {
    const userMessage = messages[messages.length - 1].content

    if (userMessage.includes('PDF') && userMessage.includes('Markdown')) {
      return {
        content: `
**需求分析**：用户想要将PDF转换为Markdown格式
**候选工具**：PdfToMarkdownTool, PdfParseTool
**推荐工具**：PdfToMarkdownTool
**选择理由**：这是专门的PDF到Markdown转换工具，能够保留文档结构
**工具参数**：
\`\`\`json
{
  "filePath": "/path/to/document.pdf",
  "includeHeaderFooter": false
}
\`\`\`
**执行计划**：使用PdfToMarkdownTool直接转换
        `,
      }
    }

    return {
      content: `
**需求分析**：处理文档
**推荐工具**：PdfToMarkdownTool
**选择理由**：最适合的工具
**工具参数**：{"filePath": "doc.pdf"}
**执行计划**：执行工具
      `,
    }
  }
}

// 模拟工具
const mockTools = new Map([
  [
    'PdfToMarkdownTool',
    {
      metadata: {
        name: 'PdfToMarkdownTool',
        description: '将PDF文件转换为Markdown格式',
        category: 'document',
      },
      execute: async (params) => {
        return { markdown: `# 转换后的内容\n\n这是从 ${params.filePath} 转换来的内容` }
      },
    },
  ],
  [
    'PdfParseTool',
    {
      metadata: {
        name: 'PdfParseTool',
        description: '解析PDF文件',
        category: 'document',
      },
      execute: async (params) => {
        return { text: `从 ${params.filePath} 解析的文本内容` }
      },
    },
  ],
  [
    'ImageOcrTool',
    {
      metadata: {
        name: 'ImageOcrTool',
        description: '图片OCR识别',
        category: 'image',
      },
      execute: async (params) => {
        return { text: `从 ${params.imagePath} 识别的文字` }
      },
    },
  ],
])

// ========== 辅助函数 ==========
function getAvailableTools(context) {
  return Array.from(context.tools.entries()).map(([name, tool]) => ({
    metadata: tool.metadata,
    execute: tool.execute.bind(tool),
  }))
}

function parseToolSelection(llmResponse) {
  const lines = llmResponse.split('\n')
  let toolName = ''
  let parameters = {}
  let reasoning = ''

  for (const line of lines) {
    if (line.includes('推荐工具') || line.includes('选择工具')) {
      toolName = line.split('：')[1]?.split('**')[0]?.trim() || ''
    }
    if (line.includes('工具参数')) {
      const jsonMatch = line.match(/```json\n([\s\S]*?)\n```/)
      if (jsonMatch) {
        try {
          parameters = JSON.parse(jsonMatch[1])
        } catch (e) {
          parameters = {}
        }
      }
    }
    if (line.includes('选择理由') || line.includes('推荐理由')) {
      reasoning = line.split('：')[1]?.trim() || ''
    }
  }

  return { toolName, parameters, reasoning }
}

// ========== 方式1: 基础集成 ==========
async function runBasicAgent() {
  console.log('\n' + '='.repeat(60))
  console.log('🔵 方式1: 基础集成智能体')
  console.log('='.repeat(60))

  console.log('🤖 基础集成智能体已初始化')

  const context = {
    llm: new MockLLM(),
    tools: mockTools,
    userInput: '帮我把这个PDF文件转换成Markdown格式',
  }

  try {
    // Plan阶段
    console.log('\n🧠 [基础] 开始规划...')

    const prompt = toolSelectionOptimizer.optimizeToolSelectionPrompt(
      context.userInput,
      getAvailableTools(context)
    )

    const response = await context.llm.chat([{ role: 'user', content: prompt }])
    const selection = parseToolSelection(response.content)
    console.log(`✅ [基础] 选择的工具: ${selection.toolName}`)

    // Act阶段
    console.log('\n⚙️ [基础] 开始执行...')

    const startTime = Date.now()
    const result = await context.tools.get(selection.toolName).execute(selection.parameters)

    // 记录使用
    toolSelectionOptimizer.recordToolUsage(
      selection.toolName,
      context.userInput,
      selection.parameters,
      {
        success: true,
        executionTime: Date.now() - startTime,
        output: result,
      }
    )

    console.log(`✅ [基础] 执行成功，耗时: ${Date.now() - startTime}ms`)

    console.log('\n✅ 基础集成演示完成！')
    console.log('📊 结果: 成功')
    console.log('📦 输出:', result.markdown.substring(0, 100) + '...')
  } catch (error) {
    console.error('❌ 基础集成演示失败:', error.message)
  }
}

// ========== 方式2: 高级集成 ==========
async function runAdvancedAgent() {
  console.log('\n' + '='.repeat(60))
  console.log('🟢 方式2: 高级集成智能体（带错误恢复）')
  console.log('='.repeat(60))

  console.log('🚀 高级集成智能体已初始化')

  const context = {
    llm: new MockLLM(),
    tools: mockTools,
    userInput: '帮我把这个PDF文件转换成Markdown格式',
  }

  try {
    // Plan阶段
    console.log('\n🧠 [高级] 开始规划...')

    const prompt = toolSelectionOptimizer.optimizeToolSelectionPrompt(
      context.userInput,
      getAvailableTools(context)
    )

    const decisionPrompt = `
基于以下推荐，请选择工具：
${prompt}

如果推荐合适，请使用推荐工具；否则说明原因并选择其他工具。
`

    const response = await context.llm.chat([{ role: 'user', content: decisionPrompt }])
    const selection = parseToolSelection(response.content)
    console.log(`✅ [高级] 选择的工具: ${selection.toolName}`)

    // Act阶段（带错误恢复）
    console.log('\n⚙️ [高级] 开始执行...')

    const startTime = Date.now()

    // 模拟第一次失败
    console.log('💥 [高级] 模拟工具执行失败...')
    const error = new Error('模拟的工具执行失败')

    console.error(`❌ [高级] 执行失败: ${error.message}`)
    console.log('🔄 [高级] 尝试使用替代工具...')

    // 尝试替代工具
    const retryPrompt = `
工具 ${selection.toolName} 执行失败：
错误：${error.message}

请基于推荐选择替代工具：
${prompt}
`

    const retryResponse = await context.llm.chat([{ role: 'user', content: retryPrompt }])
    const newSelection = parseToolSelection(retryResponse.content)

    console.log(`🔄 [高级] 尝试替代工具: ${newSelection.toolName}`)

    const result = await context.tools.get(newSelection.toolName).execute(newSelection.parameters)

    // 记录重试成功
    toolSelectionOptimizer.recordToolUsage(
      newSelection.toolName,
      context.userInput,
      newSelection.parameters,
      {
        success: true,
        executionTime: Date.now() - startTime,
        output: result,
      }
    )

    console.log(`✅ [高级] 替代工具成功，耗时: ${Date.now() - startTime}ms`)

    console.log('\n✅ 高级集成演示完成！')
    console.log('📊 结果: 成功（使用了替代工具）')
    console.log('📦 输出:', result.markdown.substring(0, 100) + '...')
  } catch (error) {
    console.error('❌ 高级集成演示失败:', error.message)
  }
}

// ========== 方式3: 生产级集成 ==========
async function runProductionAgent() {
  console.log('\n' + '='.repeat(60))
  console.log('🟣 方式3: 生产级集成智能体')
  console.log('='.repeat(60))

  const metrics = {
    totalToolCalls: 0,
    successfulCalls: 0,
    failedCalls: 0,
    avgExecutionTime: 0,
  }

  console.log('🏭 生产级集成智能体已初始化')

  const context = {
    llm: new MockLLM(),
    tools: mockTools,
    userInput: '帮我把这个PDF文件转换成Markdown格式',
    conversationHistory: [
      { role: 'user', content: '我需要处理文档' },
      { role: 'assistant', content: '我可以帮您处理文档' },
    ],
    sessionId: 'prod-session-001',
    userId: 'prod-user',
    currentTask: '文档格式转换',
  }

  try {
    // Before钩子
    console.log('\n📋 [生产] 处理请求:', context.userInput)
    console.log(`🔧 [生产] 可用工具: ${Array.from(context.tools.keys()).join(', ')}`)

    // Plan阶段
    console.log('\n🧠 [生产] 开始规划...')

    const prompt = toolSelectionOptimizer.optimizeToolSelectionPrompt(
      context.userInput,
      getAvailableTools(context),
      {
        conversationHistory: context.conversationHistory,
        currentTask: context.currentTask,
      }
    )

    const response = await context.llm.chat([{ role: 'user', content: prompt }])
    const selection = parseToolSelection(response.content)

    console.log(`✅ [生产] 选择的工具: ${selection.toolName}`)
    console.log(`💭 [生产] 推理过程: ${selection.reasoning?.substring(0, 50)}...`)

    // Act阶段
    console.log('\n⚙️ [生产] 开始执行...')
    metrics.totalToolCalls++

    const startTime = Date.now()
    const result = await context.tools.get(selection.toolName).execute(selection.parameters)
    const executionTime = Date.now() - startTime

    metrics.successfulCalls++
    metrics.avgExecutionTime =
      (metrics.avgExecutionTime * (metrics.successfulCalls - 1) + executionTime) /
      metrics.successfulCalls

    // 记录使用
    toolSelectionOptimizer.recordToolUsage(
      selection.toolName,
      context.userInput,
      selection.parameters,
      {
        success: true,
        executionTime,
        output: result,
      },
      {
        sessionId: context.sessionId,
        userId: context.userId,
        conversationHistory: context.conversationHistory,
      }
    )

    console.log(`✅ [生产] 执行成功，耗时: ${executionTime}ms`)

    // Reflect阶段
    console.log('\n🔍 [生产] 开始反思...')

    const report = toolSelectionOptimizer.getPerformanceReport()
    const accuracy = toolSelectionOptimizer.analyzeSelectionAccuracy()

    console.log(`📊 [生产] 性能指标:`, metrics)
    console.log(`📈 [生产] 选择准确率: ${(accuracy.accuracy * 100).toFixed(1)}%`)
    console.log(`💡 [生产] 改进建议:`, accuracy.improvements)

    // After钩子
    console.log('\n✅ [生产] 执行完成')
    console.log(`🎯 [生产] 最终结果: 成功`)
    console.log(`📦 [生产] 结果预览: ${JSON.stringify(result).substring(0, 100)}...`)

    console.log('\n✅ 生产级集成演示完成！')
    console.log('📊 结果: 成功')
    console.log('📈 性能指标:', metrics)
  } catch (error) {
    metrics.failedCalls++
    console.error('❌ 生产级集成演示失败:', error.message)
  }
}

// ========== 运行所有示例 ==========
async function runAllExamples() {
  console.log('🚀 开始运行3种集成方式示例...\n')

  await runBasicAgent()
  await runAdvancedAgent()
  await runProductionAgent()

  console.log('\n' + '='.repeat(60))
  console.log('🎉 所有集成方式演示完成！')
  console.log('='.repeat(60))

  console.log('\n✅ 方式1（基础集成）：适合新手，简单直接')
  console.log('   - 在plan阶段生成优化提示')
  console.log('   - 在act阶段记录工具使用')
  console.log('   - 最小化集成，快速上手')

  console.log('\n✅ 方式2（高级集成）：带错误恢复，更智能')
  console.log('   - 在plan阶段获取推荐')
  console.log('   - 在act阶段实现错误恢复')
  console.log('   - 添加替代工具逻辑')
  console.log('   - 提升系统鲁棒性')

  console.log('\n✅ 方式3（生产集成）：完整监控，生产级')
  console.log('   - 实现完整的生命周期钩子')
  console.log('   - 添加详细的日志记录')
  console.log('   - 实现性能监控和指标')
  console.log('   - 定期生成分析报告')
  console.log('   - 适合生产环境部署')

  console.log('\n🎯 选择建议：')
  console.log('  📚 新手学习：使用方式1')
  console.log('  💡 进阶开发：使用方式2')
  console.log('  🏢 生产环境：使用方式3')

  console.log('\n📚 更多资源：')
  console.log('  - 集成指南: docs/AGENT_INTEGRATION_GUIDE.md')
  console.log('  - 完整报告: docs/TOOL_SELECTION_COMPLETE_REPORT.md')
  console.log('  - 测试脚本: test-tool-selection.js')
}

runAllExamples()
