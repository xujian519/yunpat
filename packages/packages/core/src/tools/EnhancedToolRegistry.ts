...
export class EnhancedToolRegistry {
  ...
  /**
   * 输出持久化：当结果超过 maxResultSizeChars 时，同步写入临时文件并返回摘要。
   * 模型收到文件路径 + 预览，而非全量内容，节省 token。
   */
  private maybePersistResult<TOutput>(tool: EnhancedTool, result: TOutput): TOutput {
    ...
    try {
      ...
      console.error(`[ToolRegistry] Failed to persist result:`, err)
      return result
    }
    ...
    return {
      _persisted: true,
      _path: filePath,
      _sizeChars: str.length,
      _preview: str.substring(0, Math.min(2000, maxSize / 2)),
    } as TOutput
  }
 ...
 * 基础工具类
 *
 * 提供工具的默认实现，包括 Claude Code 风格的行为标记方法。
 * 子类可按需覆盖。
 */
...
> {
  ...
  isReadOnly(_input: TInput): boolean {
    return this.metadata.isConcurrencySafe ?? false
  }
...
export class ToolWrapperClass<TInput = any, TOutput = any> extends BaseTool<TInput, TOutput> {
  ...
  private isReadOnlyFn?: (input: TInput) => boolean
  ...
  constructor(
    ...
    this.isReadOnlyFn = options?.isReadOnly
  ...
  override isReadOnly(input: TInput): boolean {
    return this.isReadOnlyFn ? this.isReadOnlyFn(input) : super.isReadOnly(input)
  }

  override isConcurrencySafe(input: TInput): boolean {
    return this.isConcurrencySafeFn
      ? this.isConcurrencySafeFn(input)
      : super.isConcurrencySafe(input)
  }

  override isDestructive(input: TInput): boolean {
    return this.isDestructiveFn ? this.isDestructiveFn(input) : false
  }
 ...
 * buildTool 工厂函数
 *
 * 借鉴 Claude Code 的 buildTool 模式，从部分定义创建完整工具。
 * 自动填充安全默认值，确保所有行为标记方法可用。
 *
 * @example
 * ```ts
 ...
 *   },
 *   execute: async (input, ctx) => ({ result: 'ok' }),
 *   isReadOnly: (input) => true,  // 可选覆盖
 * })
 * ```
 */
export function buildTool<TInput = any, TOutput = any>(
  ...
  return {
    isReadOnly: def.isReadOnly ?? ((_input: TInput) => def.metadata.isConcurrencySafe ?? false),
    isConcurrencySafe:
      def.isConcurrencySafe ?? ((_input: TInput) => def.metadata.isConcurrencySafe ?? false),
    isDestructive: def.isDestructive ?? ((_input: TInput) => false),
    validateInput:
      def.validateInput ??
