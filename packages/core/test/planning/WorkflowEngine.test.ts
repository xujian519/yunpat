...
describe('WorkflowEngine', () => {
  ...
  describe('execute', () => {
    ...
    it('应该处理空ID', async () => {
      ...
      await expect(
        engine.execute({ id: '', name: '测试', steps: [] } as any)
      ).rejects.toThrow('工作流ID不能为空');
    });

    it('应该处理空名称', async () => {
      const engine = new WorkflowEngine(createMockConfig(new Map()));
      await expect(
        engine.execute({ id: 'wf-1', name: '', steps: [] } as any)
    ...
    it('应该处理空步骤', async () => {
      const engine = new WorkflowEngine(createMockConfig(new Map()));
      await expect(
        engine.execute({ id: 'wf-1', name: '测试', steps: [] })
      ).rejects.toThrow('工作流必须包含至少一个步骤');
    });
    ...
    it('应该处理步骤超时', async () => {
      const agent = createMockAgent('agent1');
      vi.spyOn(agent, 'execute').mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 100)));
    ...
    it('应该处理工作流暂停', async () => {
      ...
      await expect(
        engine.pause('nonexistent-id')
      ).rejects.toThrow('执行不存在');
    });
    ...
    it('应该处理检查点上下文无效', async () => {
      ...
      await expect(engine.resume('test-execution-id')).rejects.toThrow('检查点上下文无效: test-execution-id');
    });
    ...
    it('应该处理检查点中未找到工作流定义', async () => {
      ...
      await expect(engine.resume('test-execution-id')).rejects.toThrow('检查点中未找到工作流定义: test-execution-id');
    });
  });
});
