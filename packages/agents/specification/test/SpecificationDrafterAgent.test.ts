...
describe('SpecificationDrafterAgent', () => {
  const mockLLM = {
    chat: async ({ messages }: { messages: any[] }) => {
      ...
      if (lastMessage.includes('技术领域')) {
        return { message: { content: '本发明涉及人工智能领域，具体而言，涉及一种基于AI芯片的智能处理方法。' } };
      }
      if (lastMessage.includes('背景技术')) {
        return { message: { content: '现有技术存在效率低下的问题。传统方法无法满足实时处理需求。因此，需要提供一种更高效的处理方法。' } };
      }
      ...
      if (lastMessage.includes('摘要')) {
        return { message: { content: '本发明提供一种智能处理方法，使用AI芯片加速，效率提升显著。' } };
      }
  ...
  const mockInput = {
    ...
    claims: {
      independentClaims: [
        { claimNumber: 1, fullText: '一种智能设备...', claimType: 'device' },
      ],
      dependentClaims: [
        { claimNumber: 2, content: '如权利要求1所述...', parentClaim: 1 },
      ],
    },
  };
  ...
  it('应该验证输入参数', async () => {
    ...
    await expect(agent.execute({ inventionUnderstanding: undefined as any }))
