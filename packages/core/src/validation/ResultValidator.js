...
export class ResultValidator {
    ...
    constructor(config = {}) {
        ...
        if (config.llm && config.knowledgeBase && this.config.enableHallucinationCheck) {
            this.hallucinationDetector = new HallucinationDetector(config.llm, config.knowledgeBase, // 这里仍然是可能undefined，但在if条件中已经检查
            {
            ...
            if (content) {
                ...
                if (!qualityResult.passed) {
                    if (!qualityResult.lengthCheck.passed) {
                        errors.push(`长度检查失败: 实际 ${qualityResult.lengthCheck.actualLength} 字符` +
                    ...
                    if (!qualityResult.keywordCheck.passed) {
                        if (qualityResult.keywordCheck.missingRequired.length > 0) {
                            errors.push(`缺少必需关键词: ${qualityResult.keywordCheck.missingRequired.join(', ')}`);
                        }
                    ...
                    if (!qualityResult.completenessCheck.passed) {
                        errors.push(`内容被截断，检测到标记: ${qualityResult.completenessCheck.truncationMarker}`);
                    }
                    return {
                        valid: false,
                        errors,
    ...
    async validateWithHallucinationCheck(result, schema, options) {
        ...
        try {
            ...
            if (hallucinationReport.overallScore >= (options?.factCheckThreshold ?? 0.7)) {
                errors.push(`幻觉检测分数过高: ${(hallucinationReport.overallScore * 100).toFixed(1)}% ` +
                    `(阈值: ${((options?.factCheckThreshold ?? 0.7) * 100).toFixed(1)}%)`);
            }
            // 添加事实验证结果到警告
            if (hallucinationReport.factCheckResults.length > 0) {
                const unverifiedFacts = hallucinationReport.factCheckResults.filter((r) => r.isVerifiable && !r.isVerified);
            ...
            if (hallucinationReport.sourceAttributionIssues.length > 0) {
                const criticalIssues = hallucinationReport.sourceAttributionIssues.filter((i) => i.severity === 'critical');
    ...
    checkLength(content, requirements) {
        ...
        const passed = (!minLength || actualLength >= minLength) && (!maxLength || actualLength <= maxLength);
        return {
            passed,
            actualLength,
            minLength,
