...
export class CardPipeline {
    ...
    async loadConcepts(targetConcepts) {
        ...
        for (const [name, pages] of Object.entries(conceptPages)) {
            if (targetSet && !targetSet.has(name)) {
                // 模糊匹配：如果目标概念是索引概念的子串
                const matched = targetSet.has(name) || [...targetSet].some((t) => name.includes(t) || t.includes(name));
                if (!matched)
                    continue;
            }
            concepts.push({
     ...
     */
    async scanRelatedFiles(concept, maxFiles) {
        const pages = [];
        // 自动扫描知识库根目录下的所有子目录
