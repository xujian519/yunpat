...
export class DependencyAnalyzer {
     ...
     * 分析子目标之间的依赖关系
     */
    analyzeDependencies(subGoals, existingDependencies) {
        const nodes = new Map();
        const edges = [...(existingDependencies || [])];
        // 构建节点映射
     ...
     * 检测隐式依赖关系
     */
    detectImplicitDependencies(subGoals) {
        const dependencies = [];
     ...
     * 检查两个子目标之间是否存在依赖关系
     */
    checkDependency(fromGoal, toGoal) {
        let strength = 0;
        const reasons = [];
        // 检查显式依赖声明
     ...
     * 检查任务类型依赖
     */
    checkTaskTypeDependency(fromGoal, toGoal) {
    ...
    findCycle(nodes, edges) {
        const _parent = new Map(); // 保留用于未来实现路径追踪
        const visited = new Set();
        const recursionStack = new Set();
