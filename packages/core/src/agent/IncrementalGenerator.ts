      ...
      switch (change.type) {
        case 'modify':
          if (change.section && change.newRequirement) {
            updatedContent = await this.modifySection(
              updatedContent,
              change.section,
...
**修改后的内容**：`;
   ...
   * 添加新章节
   */
  private async addSection(content: string, section: string): Promise<string> {
...
**新章节内容**（保持与文档风格一致）：`;
   ...
   * 删除章节
   */
  private deleteSection(content: string, section: string): string {
    // 使用正则表达式删除章节
   ...
   * 构建差异分析提示
   */
  private buildDiffPrompt(originalContent: string, newRequirements: string): string {
