...
export class MermaidChartTool extends EnhancedBaseTool<any, any> {
  ...
  private generateMermaidCode(
    ...
    switch (chartType) {
      ...
      case 'mindmap':
        ...
        break
      case 'state':
        code = this.generateState(data, title)
        break
      case 'gantt':
        code = this.generateGantt(data, title)
        break
      case 'pie':
        code = this.generatePie(data, title)
        break
      default:
   ...
   * 生成状态图
   */
  private generateState(data: any, title?: string): string {
    ...
    if (data.states) {
      for (const state of data.states) {
        if (typeof state === 'string') {
          code += `    state "${state}" as ${state.replace(/\s+/g, '_')}\n`
        } else {
          code += `    state "${state.name}" as ${state.id || state.name.replace(/\s+/g, '_')}\n`
        }
      }
    }
    ...
    if (data.transitions) {
      for (const t of data.transitions) {
        const from = typeof t.from === 'string' ? t.from.replace(/\s+/g, '_') : t.from
        const to = typeof t.to === 'string' ? t.to.replace(/\s+/g, '_') : t.to
        const label = t.label || ''
        code += `    ${from} --> ${to}${label ? ': ' + label : ''}\n`
      }
    }
   ...
   * 生成甘特图
   */
  private generateGantt(data: any, title?: string): string {
    let code = 'gantt\n'
      ...
      for (const section of data.sections) {
        ...
        if (section.tasks) {
          for (const task of section.tasks) {
            const status =
              task.status === 'done' ? 'done,' : task.status === 'active' ? 'active,' : ''
            const id = task.id || task.name.replace(/\s+/g, '_')
            code += `    ${status}${task.name} :${id}, ${task.start}, ${task.duration || task.end}\n`
          }
        }
      }
    }
   ...
   * 生成饼图
   */
  private generatePie(data: any, title?: string): string {
    let code = 'pie\n'
