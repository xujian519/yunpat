...
**项目规模**：~200K 行 Rust + ~188K 行 TypeScript，18 个 Rust crate，20 个 TS 基础设施包，24 个专业 Agent，~6700 个文件。
...
**pnpm workspace，20 个基础设施包 + 24 个专业 Agent 包 + 4 个辅助包**。

#### 基础设施包

| 包名 | 行数 | 职责 |
|------|------|------|
| `@yunpat/core` | ~56K | 智能体框架核心：Agent 抽象、事件总线、生命周期、LLM 集成、DB、内存、规划 |
| `@yunpat/orchestrator` | ~6.5K | 专利 Agent 中央调度系统，组合 24 个专业 Agent |
| `@yunpat/document-tools` | ~5.4K | 文档解析工具集 — PDF、DOCX、Excel、图片、音频 |
| `@yunpat/builtin-tools` | ~4K | 内置工具集 — 文件操作、搜索、网络请求 |
| `@yunpat/tui` | ~4K | **已废弃** — 使用 Rust TUI 代替 |
| `@yunpat/unified-knowledge-graph` | ~3K | 统一知识图谱（OpenClaw + YunPat + Athena） |
| `@yunpat/mcp-server` | ~2.8K | MCP 协议服务端（bin: `yunpat-mcp`），向 Claude Desktop 暴露专利工具 |
...
| `@yunpat/rust-tools` | — | Rust 工具集桥接（向量服务、相似度、分词器，实际为 Rust 代码） |
| `@yunpat/rust-gateway` | — | Rust HTTP 网关（实际为 Rust 代码） |
| `@yunpat/python-tools` | — | Python 工具包桥接（官方文档解析器，实际为 Python 代码） |
