...
const DOMAIN_KEYWORDS: Array<{ keywords: string[]; file: string }> = [
  { keywords: ['机械', '结构', '装置', '设备', '泵', '阀', '轴承', '齿轮', '轴'], file: 'claims-domain-mechanical.md' },
  { keywords: ['电', '电路', '信号', '通信', '半导体', '芯片'], file: 'claims-domain-electrical.md' },
  { keywords: ['化学', '组合物', '化合物', '制备', '催化剂', '聚合'], file: 'claims-domain-chemical.md' },
  { keywords: ['计算机', '软件', '算法', '数据处理', '人工智能', '机器学习'], file: 'claims-domain-computer.md' },
]
...
export class ClaimGeneratorAgent extends Agent<ClaimGeneratorInput, ClaimGeneratorOutput> {
  ...
  constructor(config: any) {
    ...
    this.skillsDir =
      config.skillsDir ||
      resolve(process.cwd(), '.yunpat/skills/patent-drafting')

    // 初始化安全内容提供者（加密源优先，本地明文降级）
    this.secureProvider = config.secureProvider || new SecureContentProvider({
  ...
  protected async act(
    ...
    // 检查是否有上一次迭代的分析结果可复用
    let analysisResult = context.sharedState.get('essentialAnalysis') as EssentialAnalysisResult | undefined
    ...
    console.log(`   必要特征: ${analysisResult.analysis_table.filter(f => f.isEssential).length} 个`)
    console.log(`   非必要特征: ${analysisResult.analysis_table.filter(f => !f.isEssential).length} 个`)
...
输出严格的 JSON 格式。`
    ...
    // 构建区别特征列表
    const differences = si.prior_art_analysis?.differences || si.essential_features.map(f => f.description)
    ...
    if (!parsed) {
      ...
      return {
        ...
        analysis_table: si.essential_features.map(f => ({
          ...
          location: (f.is_essential ? 'characteristic' : 'dependent') as 'characteristic' | 'dependent',
...
- 特征部分："其特征在于" + 区别于现有技术的技术特征`
  ...
  protected async reflect(
    ...
    if (featuresToCheck.length === 0 && inventionData?.technicalSolution) {
      ...
      if (featuresToCheck.length > 0) {
        console.log(`[ClaimGeneratorAgent.reflect] 从 technicalSolution 解析 ${featuresToCheck.length} 个特征用于完整性检查`)
      }
    }
    ...
    // === 2. 非必要特征检出（基于第一步分析结果） ===
    const essentialAnalysis = context.sharedState.get('essentialAnalysis') as EssentialAnalysisResult | undefined
    ...
    if (essentialAnalysis?.analysis_table?.length) {
      for (const claim of output.claimsSet.independent_claims) {
        const found = identifyIncludedUnnecessaryFeatures(claim.full_text, essentialAnalysis.analysis_table)
    ...
    const allIssuesFound = missingFeatures.length > 0 || unnecessaryIncludedFeatures.length > 0 || !formalityIssues.passed
  ...
  private buildStructuredInput(input: ClaimGeneratorInput): StructuredInput {
    ...
    if (keyFeatures.length === 0 && inventionUnderstanding.inventionConcepts?.length) {
      keyFeatures = inventionUnderstanding.inventionConcepts.flatMap(
        (c) => c.keyFeatures || []
      )
    ...
    for (let attempt = 0; attempt <= LLM_MAX_RETRIES; attempt++) {
      try {
        ...
        if (mdParsed) {
          ...
          return this.normalizeOutput(mdParsed as unknown as Record<string, unknown>, input, startTime)
        }
      } catch (e) {
        lastError = e instanceof Error ? e : new Error(String(e))

        // 认证错误不重试
        if (lastError.message.includes('authentication') || lastError.message.includes('401') || lastError.message.includes('403')) {
          break
        }
  ...
  private normalizeOutput(
    ...
    const independent_claims: IndependentClaim[] = independentClaimsRaw.map(
      (claim, index) => ({
    ...
    const dependent_claims: DependentClaim[] = dependentClaimsRaw.map(
      (claim, index) => ({
        ...
        additional_features: Array.isArray((claim as Record<string, unknown>)?.additional_features)
          ...
          : [],
        limitation_type: strField(claim, 'limitation_type', 'further_limitation') as DependentClaim['limitation_type'],
      })
    )
    ...
    return {
      claimsSet: {
        ...
        quality_check: {
          ...
          potential_issues: fallbackFeatures.length > 0
            ? ['AI生成失败，使用简化版本']
