...
export class TaskDecomposer {
     ...
     * 创建任务对象的辅助方法
     */
    createTask(title, description, type, capabilities, tokens, duration) {
        return {
            id: uuidv4(),
            title,
            description,
     ...
     * 分解目标为层次化计划
     */
    async decompose(goal, context, options) {
     ...
     * 递归分解
     */
    async decomposeRecursive(goal, currentDepth, maxDepth, context, options) {
        // 达到最大深度，创建叶子任务
     ...
     * 应用分解规则
     */
    async applyRule(goal, rule, _currentDepth, _context, _options) {
        const subGoals = [];
     ...
     * 基于规则的分解
     */
    ruleBasedDecompose(goal, _currentDepth, _options) {
        // 通用分解策略
        const subGoals = [];
        // 1. 研究阶段
        subGoals.push({
     ...
     * 智能分解（使用LLM）
     */
    async intelligentDecompose(goal, currentDepth, context, options) {
        if (!this.config.llm) {
            // 回退到规则分解
            return this.ruleBasedDecompose(goal, currentDepth, options);
        }
     ...
     * 构建分解提示词
     */
    buildDecompositionPrompt(goal, currentDepth, options) {
...
4. 任务类型应该是 research, analysis, writing, validation, generation, review 之一`;
     ...
     * 解析分解结果
     */
    parseDecompositionResult(content, currentDepth) {
        try {
            // 尝试提取JSON
            ...
            return result.subGoals.map((sg) => ({
                ...
                status: TaskStatus.PENDING,
                estimatedDuration: (sg.tasks || []).reduce((sum, t) => sum + (t.estimatedDuration || 300), 0),
                estimatedTokens: (sg.tasks || []).reduce((sum, t) => sum + (t.estimatedTokens || 2000), 0),
            }));
        }
        catch (error) {
    ...
    parseTaskType(type) {
        ...
        return validTypes.includes(normalized) ? normalized : TaskType.WRITING;
    }
    /**
     * 推断所需能力
     */
    inferCapabilities(taskType) {
        const capabilityMap = {
    ...
    parsePriority(priority) {
        ...
        return validPriorities.includes(normalized) ? normalized : Priority.MEDIUM;
    }
    /**
     * 创建叶子子目标（达到最大深度时）
     */
    createLeafSubGoal(goal, _depth) {
        const taskId = uuidv4();
        return {
            id: uuidv4(),
            title: goal,
     ...
     * 计算统计信息
     */
    calculateStats(subGoals, dependencies) {
    ...
    mergeOptions(options) {
        return {
            ...
            enableIntelligentDecomposition: options?.enableIntelligentDecomposition ??
