 ...
 */
import { SuggestionAction, LogicalInconsistencyType, SourceAttributionIssueType, } from './hallucination-types.js';
...
export class HallucinationDetector {
    ...
    async detect(content, _context) {
        ...
        // 4. 计算总体幻觉分数
        const overallScore = this.calculateOverallScore(factCheckResults, logicalInconsistencies, sourceAttributionIssues);
        // 5. 生成改进建议
        const suggestions = this.generateSuggestions(factCheckResults, logicalInconsistencies, sourceAttributionIssues);
    ...
    generateReport(report) {
        ...
        const scoreLevel = report.overallScore < 0.3
            ? '优秀'
