 ...
 */
import { ClaimCategory, SourceType, SourceAttributionIssueType, } from './hallucination-types.js';
/**
 * 源归属验证器
 */
export class SourceAttributionValidator {
     ...
     * @returns 源归属问题列表
     */
    async validateAttribution(content, claims) {
        const issues = [];
        // 如果没有提供声明，先提取声明
        if (!claims) {
            // 简单的声明提取
     ...
     * @returns 缺少引用的问题列表
     */
    async checkMissingCitations(content, claims) {
        const issues = [];
        let issueId = 0;
        // 检查每个需要引用的声明
     ...
     * @returns 是否有引用
     */
    hasCitationNearby(content, claim) {
        if (!claim.location) {
            return false;
        }
     ...
     * @returns 可信度问题列表
     */
    async checkSourceCredibility(content) {
        const issues = [];
        let issueId = 0;
        // 提取所有引用
     ...
     * @returns 可信度分数（0-1）
     */
    async assessSourceCredibility(citation) {
        // 基于来源类型的基准可信度
     ...
     * @returns 报告文本
     */
    generateAttributionReport(issues) {
        if (issues.length === 0) {
