...
export class OAuthManager {
    ...
    constructor(config) {
        ...
        if (!this.config.tokenEncryptionKey || this.config.tokenEncryptionKey.length < 32) {
            throw new Error('OAuth 加密密钥必须提供且至少 32 字节。' +
     ...
     */
    async generateAuthorizationUrl(provider, redirectUri, scope) {
     ...
     * @returns Token 是否有效
     */
    async verifyToken(provider, accessToken) {
        const providerInstance = this.getProvider(provider);
    ...
    decryptToken(encryptedToken) {
        ...
        const iv = combined.subarray(OAuthManager.SALT_LENGTH, OAuthManager.SALT_LENGTH + OAuthManager.IV_LENGTH);
        const authTag = combined.subarray(OAuthManager.SALT_LENGTH + OAuthManager.IV_LENGTH, OAuthManager.SALT_LENGTH + OAuthManager.IV_LENGTH + OAuthManager.AUTH_TAG_LENGTH);
        const encrypted = combined.subarray(OAuthManager.SALT_LENGTH + OAuthManager.IV_LENGTH + OAuthManager.AUTH_TAG_LENGTH);
        // 从密钥和盐派生解密密钥
        const key = this.deriveKey(this.config.tokenEncryptionKey, salt);
