...
async fn create_summary(
    ...
    // `yunpat_tui::compaction=debug` does NOT match — `EnvFilter`
...
mod tests {
    ...
    fn anchor_summary_section_parses_anchor_file_into_bullets() {
        let tmpdir = tempfile::TempDir::new().unwrap();
        let yunpat_dir = tmpdir.path().join(".deepseek");
        std::fs::create_dir_all(&yunpat_dir).unwrap();
        std::fs::write(
            yunpat_dir.join("anchors.md"),
