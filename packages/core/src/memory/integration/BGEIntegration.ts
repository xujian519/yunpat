...
export interface BGEM3Config {
  /** API 基础 URL（默认 localhost:8009） */
  baseURL?: string;
  /** API 密钥 */
  apiKey?: string;
  /** 模型名称（默认 bge-m3-mlx-8bit） */
  model?: string;
  /** 批处理大小（默认 32） */
  batchSize?: number;
  /** 超时时间（毫秒） */
  timeout?: number;
  /** 缓存最大条目数（默认 1000，LRU 淘汰） */
  cacheMaxSize?: number;
}
 ...
 * 创建 BGE-M3 客户端
 */
export function createBGEM3Client(config?: BGEM3Config): BGEM3Client {
 ...
 * 默认客户端（单例）
 */
let defaultClient: BGEM3Client | null = null;

export function getDefaultBGEM3Client(): BGEM3Client {
  if (!defaultClient) {
