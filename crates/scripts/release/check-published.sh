...
if npm_version="$(npm view "yunpat-tui@${version}" version 2>/dev/null)"; then
  echo "npm yunpat-tui@${npm_version} is published."
else
  echo "npm yunpat-tui@${version} is not published." >&2
  fail=1
fi

if npm_binary_version="$(npm view "yunpat-tui@${version}" yunpatBinaryVersion 2>/dev/null)"; then
  if [[ "${npm_binary_version}" == "${version}" ]]; then
    echo "npm yunpatBinaryVersion=${npm_binary_version}."
  elif [[ "${allow_npm_binary_mismatch}" == "1" ]]; then
    echo "npm yunpatBinaryVersion=${npm_binary_version} (allowed packaging-only mismatch)."
  else
    echo "npm yunpatBinaryVersion=${npm_binary_version}, expected ${version}." >&2
    fail=1
  fi
elif [[ "${allow_npm_binary_mismatch}" == "1" ]]; then
  echo "npm yunpatBinaryVersion is absent (allowed packaging-only mismatch)."
else
  echo "npm yunpatBinaryVersion is absent for yunpat-tui@${version}." >&2
  fail=1
fi
...
if [[ "${fail}" == "0" ]]; then
  echo "Published release OK: npm yunpat-tui@${version} and ${#release_crates[@]} crates are visible."
fi
