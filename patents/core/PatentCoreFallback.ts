...
import type { TechnicalFeature, ProblemFeatureEffect, DisclosureDoc } from './PatentCoreBridge.js'

/**
 * TypeScript 降级版本：提取技术特征
 * 使用正则表达式和规则提取，不如 Rust 版本精确
 */
export async function extractFeaturesFallback(text: string): Promise<{
  features: TechnicalFeature[]
  problem_feature_effects: ProblemFeatureEffect[]
  fallback: true
}> {
  const features: TechnicalFeature[] = []
  const problem_feature_effects: ProblemFeatureEffect[] = []

  // 简单的正则表达式提取（实际项目中应使用更复杂的NLP）
  const lines = text.split('\n')
  let currentFeature: Partial<TechnicalFeature> | null = null
  ...
  for (const line of lines) {
    ...
    if (effectMatch && problem_feature_effects.length > 0) {
      problem_feature_effects[problem_feature_effects.length - 1].technical_effects.push(
        effectMatch[1]
      )
    }
  }
 ...
 */
export async function parseDisclosureFallback(
  text: string
): Promise<DisclosureDoc & { fallback: true }> {
  ...
  for (const line of lines) {
    ...
    const sectionMatch = line.match(
      /^(?:#{1,3}\s+)?(技术领域|背景技术|发明内容|具体实施方式|附图说明)/
    )
 ...
 * 使用模板生成，不如 Rust 版本智能
 */
export async function generateClaimsFallback(
  title: string,
  solution: string,
  ...
  const independentClaim = {
    id: 'claim-1',
    claim_type: 'Independent' as const,
    preamble: inventionType === 'method' ? '一种' : '一种',
    transitional_phrase:
      inventionType === 'method' ? '其特征在于，包括以下步骤：' : '其特征在于，包括：',
    elements: [
      // 简单地按句子分割solution
      ...solution
        .split(/[。；;]/)
