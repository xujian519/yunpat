      ...
      const strategy = await selector.selectBestStrategy(
        {
          hasDeviation: true,
          deviations: [
            { type: 'dependency_deviation', severity: 'medium', description: '任务被阻塞' },
          ],
        } as any,
        {
          originalPlan: { subGoals: [] },
      ...
      const strategy = await selector.selectBestStrategy(
        {
          hasDeviation: true,
          deviations: [
            { type: 'quality_deviation', severity: 'high', description: '任务过于复杂' },
          ],
        } as any,
        {
          originalPlan: {
            subGoals: [{ id: 'goal-1', status: 'pending', estimatedDuration: 1000 }],
          },
          currentState: {
            failedGoals: new Set(),
            completedGoals: new Set(),
