...
interface QualityStandardsKnowledge {
  /** 权利要求质量标准 */
  claimsStandards: string[]
  /** 说明书质量标准 */
  specificationStandards: string[]
  /** 常见错误模式 */
  commonErrors: string[]
  /** 检查清单 */
  checklists: string[]
 ...
 * 增强版质量检查计划
 */
interface EnhancedQualityCheckPlan {
  input: QualityCheckerInput
  qualityStandards?: QualityStandardsKnowledge
}

/**
 * 增强版质量检查智能体
 *
 * 在原有 QualityCheckerAgent 基础上添加了知识库检索功能：
 * - 自动检索质量标准
 * - 集成常见错误模式
 * - 增强 Prompt 构建
 * - 保持向后兼容的接口
...
> {
  protected async plan(
    ...
    // 知识图谱检索
    let qualityStandards: QualityStandardsKnowledge | undefined
    if (this.knowledgeGraph) {
      try {
        ...
        if (allResults.length > 0) {
          qualityStandards = {
            claimsStandards: allResults
              .filter((r) => r.source.includes('权利要求'))
              .map((r) => r.content),
            specificationStandards: allResults
              .filter((r) => r.source.includes('说明书'))
              .map((r) => r.content),
            commonErrors: allResults.filter((r) => r.source.includes('错误')).map((r) => r.content),
            checklists: allResults.map((r) => r.content),
          }

          console.log(`   知识图谱: 检索到 ${allResults.length} 条质量标准`)
        }
      } catch (error) {
  ...
  protected async act(
    ...
    const { input, qualityStandards } = plan
    ...
    const claimsCheck = await this.checkClaims(input, qualityStandards, context)
    console.log(`      权利要求得分: ${claimsCheck.score}/100`)

    console.log('   2️⃣ 检查说明书...')
    const specificationCheck = await this.checkSpecification(input, qualityStandards, context)
  ...
  private async checkClaims(
    ...
    if (qualityStandards && qualityStandards.claimsStandards.length > 0) {
      knowledgeContext =
        '\n\n参考质量标准：\n' +
        qualityStandards.claimsStandards
          .slice(0, 3)
          .map((s, i) => `${i + 1}. ${s.substring(0, 200)}...`)
          .join('\n')
    }
...
3. 支持性：权利要求是否得到说明书支持（A26.4）${knowledgeContext}
...
{\n  "score": 85,\n  "protection_scope": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  },\n  "clarity": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  },\n  "support": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  }\n}`
    ...
    try {
      ...
      console.warn('[EnhancedQualityCheckerAgent] 权利要求检查JSON解析失败')
    }
...
`
    ...
    let knowledgeContext = ''
    if (qualityStandards && qualityStandards.specificationStandards.length > 0) {
      knowledgeContext =
        '\n\n参考质量标准：\n' +
        qualityStandards.specificationStandards
          .slice(0, 3)
          .map((s, i) => `${i + 1}. ${s.substring(0, 200)}...`)
          .join('\n')
    }
...
3. 完整性：五大章节是否齐全${knowledgeContext}
...
{\n  "score": 85,\n  "disclosure": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  },\n  "term_consistency": {\n    "status": "pass | warning | fail",\n    "inconsistent_terms": [{ "term": "术语", "occurrences": ["章节1", "章节2"] }]\n  },\n  "completeness": {\n    "status": "pass | warning | fail",\n    "issues": ["问题1"]\n  }\n}`
    ...
    try {
      ...
      console.warn('[EnhancedQualityCheckerAgent] 说明书检查JSON解析失败')
    }
