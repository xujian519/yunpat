...
export class ComplianceChecker {
    ...
    async checkCompliance(content) {
        ...
        // 并发检查所有原则
        const results = await this.checkPrinciplesConcurrently(content, sortedPrinciples);
        // 聚合结果
        const allViolations = [];
        const allWarnings = [];
        const statistics = [];
        ...
        for (const principle of sortedPrinciples) {
            const principleResults = results.find((r) => r.violations.length > 0 || r.warnings.length > 0);
            if (principleResults) {
                statistics.push({
     ...
     * 并发检查多个原则
     */
    async checkPrinciplesConcurrently(content, principles) {
        // 如果配置了最大并发数，则分批处理
        ...
        for (const batch of batches) {
            const batchResults = await Promise.all(batch.map((principle) => this.checkSinglePrinciple(content, principle)));
     ...
     * 检查单个原则
     */
    async checkSinglePrinciple(content, principle) {
        try {
