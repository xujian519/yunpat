...
import { Logger, LogMetadata } from './logger.js'
import { OAResponderError, formatErrorForLogging } from './errors.js'
...
export async function withErrorHandling<T>(
  ...
  const {
    logError = true,
    throwOnError = true,
    defaultErrorMessage = `${context} 失败`,
    errorTransformer,
    metadata = {},
  } = options
 ...
 * ```
 */
export async function withErrorHandlingOrDefault<T>(
  operation: () => Promise<T>,
  defaultValue: T,
  context: string,
  logger: Logger,
  options: Omit<ErrorHandlingOptions, 'throwOnError'> = {}
): Promise<T> {
  try {
    return await operation()
  } catch (error) {
    const {
      logError = true,
      defaultErrorMessage = `${context} 失败，使用默认值`,
      metadata = {},
    } = options
...
export async function withErrorHandlingResult<T>(
  ...
  try {
    ...
    if (logError) {
      logger.error(defaultErrorMessage, error as Error, {
        operation: context,
        ...metadata,
      })
    }
...
export async function withRetry<T>(
  ...
  const defaultErrorMessage =
    errorOptions.defaultErrorMessage || `${context} 失败（已重试${maxRetries}次）`
 ...
 * ```
 */
export function withErrorHandlingDecorator(
  context: string,
  options: ErrorHandlingOptions = {}
): MethodDecorator {
